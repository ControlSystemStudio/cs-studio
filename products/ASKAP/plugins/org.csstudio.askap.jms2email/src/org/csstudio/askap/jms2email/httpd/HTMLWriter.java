...
public class HTMLWriter {
	 ...
	 * @param resp
	 *            Response for which to create the writer
	 * @param title
	 *            HTML title
	 * @throws Exception
	 *             on error
	...
	public void close() {
		...
		text("<a href=\"/config\">-Configuration-</a> ");
	 ...
	 * Start a table.
	 * <p>
	 * The initial column header might span more than one column. In fact, it
	 * might be the only columns header. Otherwise, the remaining column headers
	 * each span one column.
	 * 
	 * @param initial_colspan
	 *            Number of columns for the first header.
	 * @param header
	 ...
	 * 
	 * @param columns
	 *            Text for each column. Count must match the colspan of
	 *            openTable
	...
	protected void tableLine(final String... columns) {
		...
		for (String column : columns) {
			if (first) {
				first = false;
				if (odd_table_line)
					text("    <th align='left' valign='top'>" + column
							+ "</th>");
				else
					text("    <th align='left' valign='top' bgcolor='#DFDFFF'>"
							+ column + "</th>");
			} else {
				if (odd_table_line)
					text("    <td align='center' valign='top'>" + column
							+ "</td>");
				else
					text("    <td align='center' valign='top' bgcolor='#DFDFFF'>"
							+ column + "</td>");
			}
		}
		text("  </tr>");
