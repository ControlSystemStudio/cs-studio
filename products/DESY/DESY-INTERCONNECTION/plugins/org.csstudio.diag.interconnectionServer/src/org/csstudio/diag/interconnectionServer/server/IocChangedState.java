...
public class IocChangedState extends Thread{
	...
	IocChangedState ( String iocName, String iocIpAddress, boolean isRunning) {
	...
	public void run() {
		String logicalIocName = "not defined";
		/*
		 * get JMS alarm connection from InterconnectionServer class
		 * use connection like in ClientRequest()
		 */
		Connection alarmConnection = InterconnectionServer.getInstance().getAlarmConnection();
		
		/*
		 * find logical name of IOC by the physical name
		 */
		logicalIocName = getLogicalIocName ( this.iocIpAddress);
		...
		if ( isRunning()) {
			/*
			 * set channels in LDAP to running
			 */
...
epicsVME62.irm-c  mkk-irm-c       : Keine Datei Y:\directoryServer\mkk-irm-c.BootLine.dat gefunden
	...
	private void setAllRecordsToConnected ( String logicalIocName) {
		/*
	...
	private void setAllRecordsToDisconnected ( String logicalIocName) {
		/*
		 * just a convenience method
		 */
