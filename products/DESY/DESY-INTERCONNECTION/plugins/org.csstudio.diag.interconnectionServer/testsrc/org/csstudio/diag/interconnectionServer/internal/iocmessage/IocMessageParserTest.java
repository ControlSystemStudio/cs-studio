 ...
 *
 */
public class IocMessageParserTest {

	@Test
	public void parseEmptyMessage() throws Exception {
		String input = "";
		IocMessageParser parser = new IocMessageParser();
		IocMessage message = parser.parse(input);
		assertNotNull(message);
	}
	
	@Test
	public void parseMessage() throws Exception {
		String input = "TEST=foo;BAR=quux;";
		IocMessageParser parser = new IocMessageParser();
		IocMessage message = parser.parse(input);
		assertTrue(message.contains("TEST"));
		assertEquals("foo", message.getItem("TEST").getValue());
		assertTrue(message.contains("BAR"));
		assertEquals("quux", message.getItem("BAR").getValue());
	}
	
	@Test(expected=NullPointerException.class)
	public void parsingNullThrowsException() throws Exception {
		IocMessageParser parser = new IocMessageParser();
		parser.parse(null);
	}
}
