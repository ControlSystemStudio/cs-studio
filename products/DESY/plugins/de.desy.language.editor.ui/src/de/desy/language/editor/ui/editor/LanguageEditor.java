...
/**
 * This abstract class provides additional predefined behavior for source code editing of structured
 * programming languages.
 * 
 * <strong>Note:</strong> You have to register your editor on the Eclipse extension point
 * 'org.eclipse.ui.editor' to make it accessible in Eclipse. If you like to assign your editor to a
 * specified content type you have to register the editor
 * 
 * To configure the behavior redefine (override) the corresponding implementation-methods; all
 * starting with the keyword 'do'. Cause this editor inherits from TextEditor all configurations
 * possible to a TextEditor also apply to editors based in this class but be careful: please do
 * first search for a do-method before replacing behavior of the TextEditor class cause some
 ...
 * @version 0.2
 * @modifications-start
 * @modified 2008-01-20, MZ, Configuration moved from extension points to extending the editor to
 *           allow better control and to map to the SWT-default way of inheritance.
 * @modifications-end
 */
public abstract class LanguageEditor extends TextEditor {
    ...
    public static class LanguagesEditorsTokenScanner extends RuleBasedScanner {
        ...
        /**
         * Creates a token scanner. The scanner will be automatically initialized by getting the
         * rules from the rule-provider during constructor processing (the rule provider have to be
         * fully unidolized).
         * 
         * @param ruleProvider
         *            The {@link AbstractRuleProvider}.
         */
        public LanguagesEditorsTokenScanner(final AbstractRuleProvider ruleProvider) {
        ...
        private void createRules() {
            ...
            final IRule[] configuredRules = rules.toArray(new IRule[rules.size()]);
    ...
    /**
     * The current root-node to be shown in the outline-view or null if currently no outline is
     * avail.
     */
    private Node _rootNode;

    /**
     * The {@link UIEventListener} used by this editor to react on configurations changes.
     */
    private UIEventListener _uiListener;
    /**
     * The {@link UIEventListener} used by this editor to react on refresh requests.
     */
    ...
    protected LanguageEditor() {
        this.setSourceViewerConfiguration(new TextSourceViewerConfiguration() {
            ...
            public String getConfiguredDocumentPartitioning(final ISourceViewer sourceViewer) {
                final String documentPartitioning = LanguageEditor.this.doGetPartitioningId();
            ...
            public IPresentationReconciler getPresentationReconciler(
                ...
                result.setRepairer(LanguageEditor.this.getPresentationRepairer(),
                        IDocument.DEFAULT_CONTENT_TYPE);
    ...
    public void createPartControl(Composite parent) {
        ...
        // initialized.
        IPreferenceStore preferenceStore = EditorUIActivator.getDefault().getPreferenceStore();
        if (preferenceStore != null) {
            SourceViewerDecorationSupport support = this.getSourceViewerDecorationSupport(this
                    .getSourceViewer());
            ...
            support.setCursorLinePainterPreferenceKeys(PreferenceConstants.CURSOR_LINE_ENABLE
                    .getPreferenceStoreId(), PreferenceConstants.CURSOR_LINE_COLOR
                    .getPreferenceStoreId());
            support.setMarginPainterPreferenceKeys(PreferenceConstants.MARGIN_PAINTER_ENABLE
                    .getPreferenceStoreId(), PreferenceConstants.MARGIN_PAINTER_COLOR
                    .getPreferenceStoreId(), PreferenceConstants.MARGIN_COLUMNS
                    .getPreferenceStoreId());
    ...
    /**
     * Disposes this editor an unregister on UI events. This method have to be called, if you need
     * to do additional disposing, redefine {@link #doAdditionalDispose()}; the method will be
...
//        UIEvent.HIGHLIGHTING_REFRESH_REQUEST.removeListener(this._refreshListener);
    ...
    /**
     * Redefine this method to offer additional dispose of things to be disposed on editors
     * lifecycle end.
     */
     ...
     * 
     * @return The id of the partitioner or null if no partitioner should be used.
     */
    ...
    /**
     * Returns the presentation damager to be used for this editor. If you haven't a special
     * damager, we prefer to return an instance of {@link DefaultDamagerRepairer}, this instance
     * should be the same like the one returned by {@link #doGetPresentationRepairer(ITokenScanner)}
     * .
     * 
     ...
     *     this.damagerRepairer = new DefaultDamagerRepairer(codeScannerUsedForHighligthing);
     * }
     * return this.damagerRepairer;
     * </pre>
     * 
     * @param codeScannerUsedForHighligthing
     *            The code scanner used internally by the editor; this instance is passed cause the
     *            default damage-repairer requires the token scanner.
    ...
    /**
     * Returns the presentation repairer to be used for this editor. If you haven't a special
     * repairer, we prefer to return an instance of {@link DefaultDamagerRepairer}, this instance
     * should be the same like the one returned by {@link #doGetPresentationDamager(ITokenScanner)}.
     * 
     * Example (damagerRepairer may be a field):
     * 
     * <pre>
    ...
    /**
     * Returns the rule provider for text parsing while highlighting to be used for this editor. By
     * default you may create a rule provider which returns an empty list of rules, than you'll get
     * no highlighting.
     * 
     ...
     */
    protected LanguagesEditorsTokenScanner doGetTokenScanner(final AbstractRuleProvider ruleProvider) {
    ...
    /**
     * Offers the ability of additional handling after specified editor input has been set.
     */
    ...
    /**
     * Redefine this method to get informed on a committed/saved source modification.
     */
    ...
    /**
     * Sets the editor input. To offer additional behavior after the input has been set, redefine
     * {@link #doHandleDoSetInputHasBeenCalled(IEditorInput)}.
     * 
    ...
    protected final void doSetInput(final IEditorInput input) throws CoreException {
        this._codeScanner = this.doGetTokenScanner(this.getRuleProviderForHighlighting());
    ...
    public Object getAdapter(final Class adapter) {
        if (IContentOutlinePage.class.equals(adapter)) {
            if (_outlinePage == null) {
                _outlinePage = new LanguageOutlinePage(new HighlightingListener() {
                    @Override
                    protected void doHighlightRegion(final int startOffset, final int endOffset,
                            final boolean activateEditor) {
                        LanguageEditor.this.setHighlightRange(startOffset, endOffset - startOffset
                                + 1, true);
                        if (activateEditor) {
                ...
                getSourceViewer().getTextWidget().addCaretListener(_outlinePage);
            }
            this.refreshParsedTree(this.getProgressMonitor());
            return this._outlinePage;
        } else if (IMeasurementProvider.class.equals(adapter)) {
        	if (_measurementProvider == null) {
        		_measurementProvider = new IMeasurementProvider() {
    ...
    public final IPresentationDamager getPresentationDamager() {
        final IPresentationDamager result = this.doGetPresentationDamager(this._codeScanner);
    ...
    public final IPresentationRepairer getPresentationRepairer() {
        final IPresentationRepairer result = this.doGetPresentationRepairer(this._codeScanner);
    ...
    /**
     * Returns the rule provider for text parsing while highlighting to be used for this editor.
     * 
     * @return Return the rule provider to be used, not null.
     */
    public final AbstractRuleProvider getRuleProviderForHighlighting() {
        final AbstractRuleProvider result = this.doGetRuleProviderForHighlighting();
...
//        UIEvent.HIGHLIGHTING_REFRESH_REQUEST.addListener(this._refreshListener);
    ...
    protected void refreshParsedTree(final IProgressMonitor progressMonitor) {
        ...
        if (input != null) {
            final AbstractLanguageParser languageParser = this.doGetLanguageParser();
            if (languageParser != null) {
                final IDocument document = this.getDocumentProvider().getDocument(input);
                if (document != null) {
                    IFile sourceRessource = null;
                    if (input instanceof FileEditorInput) {
                        sourceRessource = ((FileEditorInput) getEditorInput()).getFile();
                    }
                    // final Runnable parserJob = new Runnable() {
                    // public void run() {
                    LanguageEditor.this._rootNode = languageParser.parse(document.get(),
                            sourceRessource, progressMonitor);
                    ...
                    if (input instanceof IFileEditorInput) {
                        ...
                        IMarker marker;
                        try {
                            fileInput.getFile().deleteMarkers(IMarker.PROBLEM, true,
                                    IResource.DEPTH_INFINITE);
                            for (final Node node : LanguageEditor.this._rootNode
                                    .getAllWarningNodes()) {
                                for (final String warning : node.getWarningMessages()) {
                                    marker = fileInput.getFile().createMarker(IMarker.PROBLEM);
                                    marker.setAttribute(IMarker.SEVERITY, IMarker.SEVERITY_WARNING);
                                    marker.setAttribute(IMarker.MESSAGE, warning);
                                    final int lineNumber = document.getLineOfOffset(node
                                            .getStatementStartOffset()) + 1;
                                    marker.setAttribute(IMarker.LINE_NUMBER, lineNumber);
                                    marker.setAttribute(IMarker.CHAR_START, node
                                            .getStatementStartOffset());
                                    marker.setAttribute(IMarker.CHAR_END, node
                                            .getStatementEndOffset());
                                }
                            }
                            for (final Node node : LanguageEditor.this._rootNode.getAllErrorNodes()) {
                                for (final String error : node.getErrorMessages()) {
                                    marker = fileInput.getFile().createMarker(IMarker.PROBLEM);
     ...
     */
    @Override
    public final void setHighlightRange(final int offset, final int length, final boolean moveCursor) {
    ...
    /**
     * 
     * @return the {@link IContentAssistProcessor} for this {@link LanguageEditor} or null for non.
     */
