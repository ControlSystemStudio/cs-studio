 ...
 * 
 *  <p>The standard {@link JndiLoginModule} already allows LDAP authentication,
 ...
 *  </ul>
 *   
 *  <p>This login module performs a 'bind' to LDAP without trying
 *  to read any specific attributes.
 *  
 *  <p>JAAS config file parameters:
 *  <ul>
 ...
 *  </ul>
 *  
 *  <p>Fundamentally, this {@link LDAPBindLoginModule}
 *  could directly be used by JAAS as a {@link LoginModule}.
 *  Under Eclipse, however, the code invoking JAAS may be in a different class loader.
 *  
 *  <p>The "org.eclipse.equinox.security.loginModule" extension point is therefore used
 *  to make this {@link LoginModule} accessible from 
 *  <code>org.eclipse.equinox.security.auth.module.ExtensionLoginModule</code>
 *  as "extensionId=org.csstudio.security.ldapBind".
 *
 *  <p><b>Certificate Issues:</b>
 *  LDAP access may fail with a rather cryptic error
 *  <pre>
 ...
 *  </pre>
 *  
 *  More detail can be obtained when running with 
 *  <pre>-Djavax.net.debug=all</pre>
 *  
 *  Fix: Add required certificate to the JRE:
 *  <pre>
 *  cd .../jdk...
 *  bin/keytool -import -keystore jre/lib/security/cacerts -alias whatever -file /path/to/your/cacert.pem 
 *  </pre>
 *  
 *  The keytool will ask for a keystore password.
 *  The default is 'changeit', and since write access to the keystore is already limited
 *  by the operating system, you may decide to leave it at that.
 * 
 *  @author Kay Kasemir
...
public class LDAPBindLoginModule implements LoginModule
{
    ...
    final private static String DEBUG_TAG = "debug";

    /** JAAS config argument for LDAP server URL
     *  Using same name as JndiLoginModule to ease transition
     */
    final private String URL_TAG = "user.provider.url";

    /** JAAS config argument for DN format */
    final private static String USER_DN_FORMAT_TAG = "user.dn.format";

    /** Subject to configure on success */
    private Subject subject;
    
    /** Callback handler to use for obtaining name, password */
    ...
    private String[] getUserPassword() throws LoginException
    {
        ...
        {
            Logger.getLogger(getClass().getName()).log(Level.WARNING, "Cannot get user, password", ex);
