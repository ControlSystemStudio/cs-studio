 ...
 */
package org.csstudio.platform.ui.internal.connectionview;
...
public final class ConnectionStateView extends ViewPart {
		...
		public void mouseUp(MouseEvent e) {
			ViewerFilter filter = new ViewerFilter() {
				...
				public boolean select(Viewer viewer, Object parentElement,
						Object element) {
					IConnector s = (IConnector) element;
	...
	class FilterButton {
		private Button _swtButton;
		private Set<ConnectionState> _connectionStates;
		private String _baseLabel;

		public FilterButton(Composite parent, int style, String label,
				ConnectionState... connectionStates) {
			...
			for (ConnectionState s : connectionStates) {
		...
		public void refreshLabel(List<IConnector> statistics) {
			assert statistics != null;

			int count = 0;

			if (statistics != null) {
				for (IConnector s : statistics) {
	...
	public void createPartControl(final Composite parent) {
		...
		_viewer.setContentProvider(new IStructuredContentProvider(){
			...
			public Object[] getElements(final Object parent) {
				List<IConnector> statistics =((IProcessVariableConnectionService) parent)
						.getConnectors();
				...
				new IProcessVariableAdressProvider() {
					public List<IProcessVariableAddress> getProcessVariableAdresses() {
						...
						IStructuredSelection sel = (IStructuredSelection) viewer
								.getSelection();
						Iterator<IProcessVariableAdressProvider> it = sel
								.iterator();
					...
					public IProcessVariableAddress getPVAdress() {
						IStructuredSelection sel = (IStructuredSelection) viewer
								.getSelection();
						IProcessVariableAdressProvider pvProvider = (IProcessVariableAdressProvider) sel
								.getFirstElement();
	...
	class ChangeSortDirectionListener implements SelectionListener {
		...
		public ChangeSortDirectionListener(SortDirection sortDirection) {
