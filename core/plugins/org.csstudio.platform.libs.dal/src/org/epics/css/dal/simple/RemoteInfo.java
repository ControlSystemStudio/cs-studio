...
/**
 * <p>
 * RemoteInfo class presents target in remote system. It briefly follows URI notation, but does not
 * necessary conform with the specifications. RemoteInfo consists of following basic components:
 * </p>
 * <ul>
 * <li>remote name, in URI notation this is path with authority. RemoteInfo does not try to
 * interpret this part or modify (like URI escaping), but leaves to plug implementation to handle
 * the remote name as pleased.</li>
 ...
 * <li>changeType, used as a hint for the implementation for which type of change should be
 * monitored (not implemented by all implementations).</li>
 * </ul>
 * <p>
 * In current DAL implementation only type and remote name are handled by DAL. changeType is only
 * handled by the EPICS implementation. Characteristic and query part are there for future
 * compatibility.
 * </p>
 * 
 * @author Igor Kriznar
...
public final class RemoteInfo {
    ...
    /**
     * Returns the connection type name of this RemoteInfo, corresponds to schema part of URI
     * specifications.
     * 
     * @return the connection type name of this RemoteInfo, or <code>null</code> if type is
     *         <code>null</code> or does not conform to DAL plug type declaration
     */
    ...
    /**
     * Returns the DAL plug name of this RemoteInfo. DAL Plug type is defined trough connection
     * type, which begins with 'DAL-'. Plug type corresponds to connection type stripped of the
     * 'DAL-' part.
     * 
     * @return the DAL plug name of this RemoteInfo, or <code>null</code> if type is
     *         <code>null</code> or does not conform to DAL plug type declaration
     */
    public String getPlugType() {
        if (connectionType != null && connectionType.length() > DAL_TYPE_PREFIX.length()
                && connectionType.toUpperCase().startsWith(DAL_TYPE_PREFIX)) {
    ...
    public static RemoteInfo remoteInfoFromString(String name, final String defaultType) {
