...
public abstract class AbstractConnector implements IConnector,
	...
	public final void addProcessVariableValueListener(String characteristicId,
		...
		synchronized (_weakListenerReferences) {
			_weakListenerReferences.add(new ListenerReference(characteristicId,
					listener));
		}
	 ...
	 */
	public final boolean removeProcessVariableValueListener(
			IProcessVariableValueListener listener) {
		synchronized (_weakListenerReferences) {
	 ...
	 *            return type
	 */
	public final void getValueAsynchronously(
	 ...
	 * @return the current value or null
	 */
	public final void getCharacteristicAsynchronously(
			final String characteristicId, final ValueType valueType,
			final IProcessVariableValueListener listener) {
		try {
			doGetCharacteristicAsynchronously(characteristicId, valueType,
					listener);
		} catch (Exception e) {
			CentralLogger.getInstance().error(null, e);
		}
	 ...
	 *             an arbitrary exception
	 */
	protected abstract void doGetCharacteristicAsynchronously(
			String characteristicId, ValueType valueType,
	 ...
	 *             an arbitrary exception
	 */
	protected abstract Object doGetCharacteristicSynchronously(
			String characteristicId, ValueType valueType) throws Exception;
	 ...
	 *             an arbitrary exception
	 */
	protected abstract void doGetValueAsynchronously(
			final IProcessVariableValueListener listener) throws Exception;;
	 ...
	 *             an arbitrary exception
	 */
	protected abstract boolean doSetValueSynchronously(Object value)
			throws Exception;
	 ...
	 */
	protected final void doForwardConnectionStateChange(
	 ...
	 */
	protected final void doForwardValue(final Object value,
		...
		if (value != null) {
			...
			execute(new IInternalRunnable() {
				...
				public void doRun(IProcessVariableValueListener valueListener) {
					valueListener.valueChanged(ConverterUtil.convert(value,
	 ...
	 *            the characteristic id
	 */
	protected final void doForwardCharacteristic(
			final Object characteristicValue, final Timestamp timestamp,
			...
			execute(new IInternalRunnable() {
				public void doRun(IProcessVariableValueListener valueListener,
					...
					if (cId != null && cId.equals(characteristicId)) {
						valueListener.valueChanged(characteristicValue,
								timestamp);
					}
				}
	...
	static class ListenerReference {
		...
		public ListenerReference(String characteristic,
				IProcessVariableValueListener<?> listener) {
			_characteristicId = characteristic;
			_listener = new WeakReference<IProcessVariableValueListener>(
					listener);
		}
