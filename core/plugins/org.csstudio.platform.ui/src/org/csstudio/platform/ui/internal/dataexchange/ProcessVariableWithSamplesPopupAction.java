...
{    
    ...
    public void run(IAction action)
    {
        ...
        Vector<IProcessVariableWithSample> pv_names = new Vector<IProcessVariableWithSample>();
        while (iter.hasNext())
        {
            Object element = iter.next();
            IProcessVariableWithSample pv = null;
            if (element instanceof IProcessVariableWithSample)
                pv = (IProcessVariableWithSample) element;
            else if (element instanceof IAdaptable)
                pv = (IProcessVariableWithSample)
                ((IAdaptable)element).getAdapter(IProcessVariableWithSample.class);
            if (pv != null)
                pv_names.add(pv);
        }
        if (pv_names.size() > 0)
        {
            IProcessVariableWithSample pvs[] = 
                new IProcessVariableWithSample[pv_names.size()];
     ...
     */
    abstract public void handlePVs(IProcessVariableWithSample pv_names[]);
}
