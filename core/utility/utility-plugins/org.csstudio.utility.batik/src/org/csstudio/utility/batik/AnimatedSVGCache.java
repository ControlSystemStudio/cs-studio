...
/**
 * Cache that stores images until the simple time of all {@link TimedElement} of
 * the current {@link TimedDocumentRoot} has been reset.
 * 
...
public class AnimatedSVGCache {
	...
	/**
	 * Maximum number of images to be handled by the cache. If more, the cache
	 * is flushed.
	 */
	private final int maxSize;

	/**
	 * Interface to be notified of new images when the cache is taking over
	 * standard Batik management.
	 */
	...
	private List<Entry> entries = Collections
			.synchronizedList(new LinkedList<Entry>());
	...
	public AnimatedSVGCache(final Display swtDisplay,
			final TimedDocumentRoot timedDocumentRoot,
	...
	public Image addImage(BufferedImage awtImage) {
		...
		if (!initialized) { // Initialize
			...
			for (TimedElement te : timedDocumentRoot.getChildren()) {
				if (timeHandlers.get(te) != null
						&& timeHandlers.get(te).update(te)
	...
	private boolean hasRepeated() {
		...
		for (TimedElement te : timedDocumentRoot.getChildren()) {
			if (timeHandlers.get(te) != null
					&& !timeHandlers.get(te).hasRepeated()) {
				return false;
			}
		}
		return true;
	...
	public void startProcessing() {
		...
		Runnable animationTask = new Runnable() {
			...
			public void run() {
				...
				if (Math.abs(currentTime - lastUpdateTime) >= currentEntry
						.getWaitTime()) {
					if (listener != null) {
	...
	/**
	 * Cache entry with an {@link Image} and a delay to wait before displaying
	 * it.
	 */
	private class Entry {
	...
	private class TimedElementHandler {
		...
		@Override
		public String toString() {
			return "TimedElementHandler [previousTime=" + previousTime
					+ ", lastTime=" + lastTime + ", simpleDur=" + simpleDur
					+ ", repeated=" + repeated + ", currentWaitTime="
					+ currentWaitTime + ", waitTimeSum=" + waitTimeSum + "]";
		}
