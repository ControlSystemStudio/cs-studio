...
public class RegExToMatchStateStatements_Test extends TestCase {
    ...
    protected void setUp() throws Exception {
        ...
        this._pattern = Pattern.compile("(state\\s+)([a-zA-Z][0-9a-zA-Z]*)(\\s*\\{)([\\s\\S]*)(\\}[;]?)");
    }

    @Test
    public void testsimpleRegEx() {
        final String when1 = " when (pvGet(xyz) == 42 ) { pvPut(computerOff, 1); pvPut(knockOffWork, TRUE); } state CallItADay  ";
        final Matcher matcher = this._pattern.matcher("state Feierabend {" + when1 + "}");
    ...
    public void testsimpleRegExMultipipleWhens() {
        ...
        final Matcher matcher = this._pattern.matcher("state Feierabend {" + when1 + when2 + "}");
        Assert.assertTrue(matcher.matches());
