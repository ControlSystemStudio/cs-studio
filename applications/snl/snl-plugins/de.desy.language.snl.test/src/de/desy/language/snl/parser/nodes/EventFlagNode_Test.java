...
public class EventFlagNode_Test extends TestCase {
    ...
    public void testNode() {
        ...
        Assert.assertEquals("event flag: eventFlagName", node.humanReadableRepresentation());
    }

    public void testSynchronizedVariable() {
        final EventFlagNode node = new EventFlagNode("eventFlagName", 23, 42);
        final SyncStatementNode syncNode = new SyncStatementNode("eventFlagName", "variableName", 23, 42);
        ...
        Assert.assertEquals("variableName", ((SyncStatementNode) childrenNodes[0]).getContent());

        Assert.assertEquals("event flag: eventFlagName synchronized with variableName",
