...
public class RegExToMatchMultiLineComment_Test extends TestCase {
    ...
    public void testsimpleRegEx() {
        final Matcher matcher = this._pattern.matcher("/* This is a comment in a program\nHere it goes furher */");
        Assert.assertTrue(matcher.matches());

        Assert.assertEquals(3, matcher.groupCount());
        Assert.assertEquals("/*", matcher.group(1)); // the single line
                                                     // embedded c beginning
        Assert.assertEquals(" This is a comment in a program\nHere it goes furher ", matcher.group(2)); // the
                                                                                                        // c
                                                                                                        // statement
    ...
    public void testsimpleRegEx_FailWithMissingStart() {
        final Matcher matcher = this._pattern.matcher("/ This is a comment in a programm */");
