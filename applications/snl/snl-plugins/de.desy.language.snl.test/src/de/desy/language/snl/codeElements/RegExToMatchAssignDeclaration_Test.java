...
public class RegExToMatchAssignDeclaration_Test extends TestCase {
    ...
    protected void setUp() throws Exception {
        ...
        this._pattern = Pattern.compile("(assign\\s+)([a-zA-Z][0-9a-zA-Z]*)(\\s+to\\s+)(\"[\\s\\S]*\")(\\s*;)");
    }
    ...
    public void testsimpleRegEx() {
        ...
        Assert.assertEquals(" to ", matcher.group(3)); // to with surrounding
                                                       // whitespaces
        Assert.assertEquals("\"test://ChannelName:VarName.VAL[hallo=welt]\"", matcher.group(4)); // channel
                                                                                                 // name
                                                                                                 // with
                                                                                                 // surrounding
                                                                                                 // quotes
    ...
    public void testsimpleRegExWithEmptyChannel() {
        final Matcher matcher = this._pattern.matcher("assign testVariable to \"\";");
        Assert.assertTrue(matcher.matches());
    ...
    public void testsimpleRegExWithNoVariable() {
        final Matcher matcher = this._pattern.matcher("assign to \"test://ChannelName:VarName.VAL[hallo=welt]\";");
