 ...
 *
 * Attention: This pattern does not match a statement with following chars behind the statement.
 *
 * The pattern:
 *
 * <pre>
...
public class RegExToMatchWhenStatements_Test extends TestCase {
    ...
    protected void setUp() throws Exception {
        super.setUp();
        this._pattern = Pattern.compile(RegExToMatchWhenStatements_Test.WHEN_PATTERN);
    }

    @Test
    public void testSimpleRegEx() {
        final Matcher matcher = this._pattern.matcher(
                "when (pvGet(xyz) == 42 ) { pvPut(computerOff, 1); pvPut(knockOffWork, TRUE); } state CallItADay");
        ...
        Assert.assertEquals(") {", matcher.group(4)); // closing brace of
                                                      // condition
        // plus block begin
        Assert.assertEquals(" pvPut(computerOff, 1); pvPut(knockOffWork, TRUE); ", matcher.group(5)); // statement block
                                                                                                      // without
                                                                                                      // surrounding
        // braces
    ...
    public void testSimpleRegEx_EmptyBlock() {
        final Matcher matcher = this._pattern.matcher("when (pvGet(xyz) == 42 ) {} state CallItADay");
        Assert.assertTrue(matcher.matches());
    ...
    public void testSimpleRegEx_FailureWithMissingStateStatement() {
        final Matcher matcher = this._pattern.matcher("when (pvGet(xyz) == 42 ) {} CallItADay");
        Assert.assertFalse(matcher.matches());
