...
public class VariableNode_Test extends TestCase {
    ...
    public void testNode() {
        final VariableNode node = new VariableNode("variableName", "long", 23, 42, true);
        ...
        Assert.assertEquals("Variable: variableName (Type: long)", node.humanReadableRepresentation());
        Assert.assertTrue(node.isArray());
    }

    public void testAssignVariable() {
        final VariableNode node = new VariableNode("variableName", "long", 23, 42, false);
        final AssignStatementNode assignStatementNode = new AssignStatementNode("variableName",
                "epics://krykWeather.temp_ai.VAL", 23, 42, false);

        Assert.assertFalse(node.isAssigned());
        node.setAssignedChannel(assignStatementNode);

        Assert.assertTrue(node.isAssigned());
        Assert.assertEquals("epics://krykWeather.temp_ai.VAL", node.getAssignedChannelName());
        ...
        Assert.assertEquals("epics://krykWeather.temp_ai.VAL", ((AssignStatementNode) childrenNodes[0]).getContent());

        Assert.assertEquals("Variable: variableName (Type: long, assigned to \"epics://krykWeather.temp_ai.VAL\")",
    ...
    public void testMonitoredVariable() {
        final VariableNode node = new VariableNode("variableName", "long", 23, 42, false);
        final MonitorStatementNode monitorNode = new MonitorStatementNode("variableName", 23, 42);
        ...
        Assert.assertEquals("Variable: variableName (Type: long, monitored)", node.humanReadableRepresentation());
    }
