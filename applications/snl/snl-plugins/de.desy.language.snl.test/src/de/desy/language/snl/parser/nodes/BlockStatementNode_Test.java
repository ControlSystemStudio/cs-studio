...
public class BlockStatementNode_Test extends TestCase {
    ...
    public void testBlockStatementNode() {
        final BlockStatementNode blockStatementNode = new BlockStatementNode(" pvPut(); // XXX", 23, 42);
        Assert.assertTrue(blockStatementNode.hasContent());
        Assert.assertEquals(" pvPut(); // XXX", blockStatementNode.getContent());
        Assert.assertEquals("{}", blockStatementNode.getSourceIdentifier());
        Assert.assertEquals("statement block", blockStatementNode.getNodeTypeName());
