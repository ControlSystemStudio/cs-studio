...
/**
 * Dialog to add a subnet to the control Subnet preferences page.
 *
...
class AddSubnetDialog extends TitleAreaDialog {
	 ...
	 */
	AddSubnetDialog(final Shell parentShell) {
		super(parentShell);
	}
	...
	protected void okPressed() {
		try {
			final InetAddress address = InetAddress.getByName(_networkAddress.getText());
			final InetAddress netmask = InetAddress.getByName(_netmask.getText());
			_result = _networkAddress.getText()+"/"+_netmask.getText();
	...
	protected Control createDialogArea(final Composite parent) {
		...
		final Composite contents = new Composite(parentComposite, SWT.NULL);
        ...
        final Label addressLabel = new Label(contents, SWT.NULL);
        ...
        final Label netmaskLabel = new Label(contents, SWT.NULL);
	...
	private void checkValid() {
		final boolean addressValid = _networkAddress.getText().matches(IPV4_ADDRESS_REGEX);

		boolean netmaskValid;
		if (_netmask.getText().matches(IPV4_ADDRESS_REGEX)) {
			try {
				final InetAddress netmask = InetAddress.getByName(_netmask.getText());
//				netmaskValid = netmask.isMulticastAddress();
				netmaskValid = true;
	...
	private void autocompleteNetmask() {
		...
		try {
			...
			if (addrBytes.length == 4) {
				final StringBuilder netmask = new StringBuilder();
				for (int i = 0; i < 4; i++) {
