...
public class MessagePropertyTypeContent {
	...
	public static void readMessageTypes() {
		...
		Connection _databaseConnection = DBConnection.getInstance().getConnection();
		try {
			PreparedStatement getMessages = _databaseConnection.prepareStatement(
					"select mpt.id, mpt.name from msg_property_type mpt");
			...
			CentralLogger.getInstance().error(new MessagePropertyTypeContent(),
