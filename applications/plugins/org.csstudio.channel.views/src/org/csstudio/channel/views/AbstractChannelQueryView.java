...
public abstract class AbstractChannelQueryView<T extends AbstractChannelQueryResultWidget> extends ViewPart {
	...
	public void createPartControl(Composite parent) {
		...
		inputBar = new ChannelQueryInputBar(parent, SWT.NONE, 
				Activator.getDefault().getDialogSettings(), viewShortName + "query");
		...
		if (toConfigurableWidget(channelQueryWidget) != null) {
			...
			configureButton.setImage(ResourceManager.getPluginImage("org.csstudio.channel.widgets", "icons/gear-16.png"));
			...
			configureButton.addSelectionListener(new SelectionAdapter() {
				...
				public void widgetSelected(SelectionEvent e) {
					toConfigurableWidget(channelQueryWidget).openConfigurationDialog();
				}
			});
		} else {
		...
		if (toISelectionProvider(channelQueryWidget) != null) {
			PopupMenuUtil.installPopupForView(channelQueryWidget, getSite(), toISelectionProvider(channelQueryWidget));
		}
