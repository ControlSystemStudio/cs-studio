...
*/
	...
	public static Telegram readNextTelegram(DataInputStream inStream) throws Exception
	{
		...
		Log.log(inStream, Log.INFO, "incoming av= " + iAvailable + "head dump " + toHexDump(baHead));
		...
		Log.log(inStream, Log.INFO, "incoming body dump " + toHexDump(baBody));
		...
		switch (telegramID)
		{
			...
			case TELID_ALARM_REPLY:
				...
				break;
			default:
				Log.log(inStream, Log.WARN, "incoming message no/len=(" + telegramCnt + "/"+ telegramLen 
	...
	public static String toHexDump(byte[] ba)
	{
		...
		{
			str += Integer.toHexString((int)((ba[i] & 0xF0) >> 4));
			str += Integer.toHexString((int)(ba[i] & 0xF));
			str += " ";
			if ((i+1) % 16 == 0)
				str += "\n";
			else if ((i+1) % 8 == 0)
