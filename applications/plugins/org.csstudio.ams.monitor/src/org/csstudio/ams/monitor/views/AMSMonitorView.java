...
public class AMSMonitorView extends ViewPart implements SelectionListener
{
	...
	private final int[] intervals = new int[]{-1,200,500,1000,2000,5000,10000};
    ...
    public void showBusy(boolean busy)
    {
    	if(busy)
    	{
    		Cursor busyCursor = mainComposite.getParent().getDisplay().getSystemCursor(SWT.CURSOR_APPSTARTING);  
	...
	public void widgetSelected(SelectionEvent e)
	{
		...
		else if(e.getSource() == cmdRefresh)
		{
			if(!optLast.getSelection())
			{
				...
				{
					MessageBox msg = new MessageBox(mainComposite.getShell(), SWT.ICON_ERROR);
					msg.setMessage(NLS.bind(Messages.AMSMonitorView_Error_InputFormatTime_Msg, maskTime));  
					msg.setText(Messages.AMSMonitorView_Error_InputFormattime_Title);
					msg.open();
					return;
				}
				...
				new Thread(new MyRunnable()
				{
					public void run()
					{
						try
						{
							synchronized(historyList)
							{
								final List<HistoryTObject> list = 
									HistoryDAO.selectList(MonitorPlugin.getConnection(), periodStartfinal, periodEndfinal, 2000);
		...
		{	
			...
			autoLoader.isAutoLoading = optLast.getSelection() && autoLoadInterval > -1;
	...
	{	
		...
		optLast.setLayoutData(Utils.getGridData(-1, -1, 1, 1, SWT.BEGINNING, SWT.CENTER, false, false));
		lblLastCount = new Label(filterComposite, SWT.NONE);
		lblLastCount.setLayoutData(Utils.getGridData(-1, -1, 1, 1, SWT.BEGINNING, SWT.CENTER, false, false));
		txtLastCount = new Text(filterComposite, SWT.BORDER | SWT.SINGLE);
		txtLastCount.setLayoutData(Utils.getGridData(100, -1, 1, 1, SWT.BEGINNING, SWT.CENTER, false, false));
		new Label(filterComposite, SWT.NONE);	//dummy
		cboInterval = new Combo(filterComposite, SWT.READ_ONLY);
		cboInterval.setLayoutData(Utils.getGridData(80, -1, 3, 1, SWT.BEGINNING, SWT.CENTER, false, false));
		
		optPeriod = new Button(filterComposite, SWT.RADIO);
		optPeriod.setLayoutData(Utils.getGridData(-1, -1, 1, 1, SWT.BEGINNING, SWT.CENTER, false, false));
		lblPeriodStart = new Label(filterComposite, SWT.NONE);
		lblPeriodStart.setLayoutData(Utils.getGridData(-1, -1, 1, 1, SWT.BEGINNING, SWT.CENTER, false, false));
		txtPeriodStart = new Text(filterComposite, SWT.BORDER | SWT.SINGLE);
		txtPeriodStart.setLayoutData(Utils.getGridData(100, -1, 1, 1, SWT.BEGINNING, SWT.CENTER, false, false));
		lblPeriodEnd = new Label(filterComposite, SWT.NONE);
		lblPeriodEnd.setLayoutData(Utils.getGridData(-1, -1, 1, 1, SWT.BEGINNING, SWT.CENTER, false, false));
		txtPeriodEnd = new Text(filterComposite, SWT.BORDER | SWT.SINGLE);
		txtPeriodEnd.setLayoutData(Utils.getGridData(100, -1, 1, 1, SWT.BEGINNING, SWT.CENTER, false, false));
		cmdClear = new CommandButton(filterComposite, CommandButton.CLEAR);
		cmdClear.setLayoutData(Utils.getGridData(-1, -1, 1, 1, SWT.END, SWT.CENTER, true, false));
	...
	private void createTable() 
	{
		...
		column.setText(Messages.getString("MonitorTable_tbl_Colum"+idx));
		column.setWidth(120);
		column.addSelectionListener(new SortSelectionAdapter(TableSorter.MSGEVENTTIME));

		column = new TableColumn(tblMonitor, SWT.LEFT, idx++);
		column.setText(Messages.getString("MonitorTable_tbl_Colum"+idx));
		column.setWidth(450);
		column.addSelectionListener(new SortSelectionAdapter(TableSorter.DESCRIPTION));

		column = new TableColumn(tblMonitor, SWT.LEFT, idx++);
		column.setText(Messages.getString("MonitorTable_tbl_Colum"+idx));
		column.setWidth(80);
		column.addSelectionListener(new SortSelectionAdapter(TableSorter.ACTIONTYPE));
		
		column = new TableColumn(tblMonitor, SWT.LEFT, idx++);
		column.setText(Messages.getString("MonitorTable_tbl_Colum"+idx));
		column.setWidth(60);
		column.addSelectionListener(new SortSelectionAdapter(TableSorter.USERREF));
		
		column = new TableColumn(tblMonitor, SWT.LEFT, idx++);
		column.setText(Messages.getString("MonitorTable_tbl_Colum"+idx));
		column.setWidth(110);
		column.addSelectionListener(new SortSelectionAdapter(TableSorter.USERNAME));
		
		column = new TableColumn(tblMonitor, SWT.LEFT, idx++);
		column.setText(Messages.getString("MonitorTable_tbl_Colum"+idx));
		column.setWidth(70);
		column.addSelectionListener(new SortSelectionAdapter(TableSorter.DESTTYPE));
		
		column = new TableColumn(tblMonitor, SWT.LEFT, idx++);
		column.setText(Messages.getString("MonitorTable_tbl_Colum"+idx));
		column.setWidth(110);
		column.addSelectionListener(new SortSelectionAdapter(TableSorter.DESTADRESS));

		column = new TableColumn(tblMonitor, SWT.LEFT, idx++);
		column.setText(Messages.getString("MonitorTable_tbl_Colum"+idx));
		column.setWidth(60);
		column.addSelectionListener(new SortSelectionAdapter(TableSorter.GROUPREF));
		
		column = new TableColumn(tblMonitor, SWT.LEFT, idx++);
		column.setText(Messages.getString("MonitorTable_tbl_Colum"+idx));
		column.setWidth(80);
		column.addSelectionListener(new SortSelectionAdapter(TableSorter.GROUPNAME));
		
		column = new TableColumn(tblMonitor, SWT.LEFT, idx++);
		column.setText(Messages.getString("MonitorTable_tbl_Colum"+idx));
		column.setWidth(80);
		column.addSelectionListener(new SortSelectionAdapter(TableSorter.RECIEVERPOS));
	}
	...
	private class HistoryLoader extends Thread
	{
		...
		public void run()
		{
			while(running)
			{
				try
				{
					...
					if(!isAutoLoading)
						continue;
					Log.log(Log.INFO, "AMSMonitor.AutoLoading Check for new data...");
					...
					{					
						...
						Log.log(Log.INFO, "AMSMonitor.AutoLoading Check for new data...Found " + newData.size());
						...
						mainComposite.getDisplay().syncExec(new Runnable() 
						{
							public void run()
							{
								if(running)
								{
									tblMonitorViewer.refresh();
									if (bAutoscroll)
										tblMonitor.setSelection(0);				// for autoscrolling, if new history entries
								}
							}
						});
					}
		...
		public List<HistoryTObject> getHistory(int maxCount)
		{
			...
			try
			{
				...
				array = HistoryDAO.selectList(MonitorPlugin.getConnection(), autoLoadCount, lastHistoryID);
	...
	public class HistoryLabelProvider extends LabelProvider implements ITableLabelProvider
	{
		...
		public String getColumnText(Object element, int columnIndex) 
		{
			...
			case 7:
				if (item.getActionType() == null || item.getActionType().length() == 0)
					return "";
	...
	private class SortSelectionAdapter extends SelectionAdapter
	{
		...
		public void widgetSelected(SelectionEvent e) 
		{
			...
			tblMonitorViewer.setSorter(new TableSorter(fieldID,fieldID == TableSorter.HISTORYID ? -1 : sortDirection));
		}
	}
	...
	public class TableSorter extends ViewerSorter 
	{
		...
		public int compare(Viewer viewer, Object o1, Object o2) {
			...
			switch (fieldID) {
				...
				case MSGEVENTTIME :
					return compareString(val1.getMsgEventtime(), val2.getMsgEventtime());		//Format noch nicht bekannt
				case DESCRIPTION :
					return compareString(val1.getDescription(), val2.getDescription());
				case ACTIONTYPE :
				...
				case RECIEVERPOS :
					return compareNumber(val1.getReceiverPos(), val2.getReceiverPos());
				case USERREF :
