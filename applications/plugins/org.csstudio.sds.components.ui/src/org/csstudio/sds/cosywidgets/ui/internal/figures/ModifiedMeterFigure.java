 ...
 * 
 */
public final class ModifiedMeterFigure extends RectangleFigure {
	/**
	...
	private FrameFigure _frame;

	private Pointer _pointer;

	public ModifiedMeterFigure() {
		...
		_frame = new FrameFigure();
		add(_frame);
		setConstraint(_frame, new Rectangle(0, 0, 100, 100));

		_pointer = new Pointer();
		add(_pointer);
		setConstraint(_pointer, new Rectangle(0, 0, 100, 100));

	}

	private void doRefreshPositions() {
		setConstraint(_frame, calculateScaleConstraints());
		_frame.invalidate();

		setConstraint(_pointer, calculateScaleConstraints());
		_pointer.invalidate();
	}

	/**
	 * Calculate the layout constraints for the frame.
	 * 
	...
	class FrameFigure extends Panel {
		...
		private void drawBackground(Graphics gfx) {
			int img_width = getBounds().width;

			gfx.setForegroundColor(ColorConstants.red);
		...
		private void drawScale(Graphics gfx) {
			...
			for (curr = min_value + minor_step; curr <= max_value - minor_step; curr += minor_step) {
				curr_angle = 90.0 + (double) inner_angle / 2
						- ((double) inner_angle / (max_value - min_value))
						* (curr - min_value);
				ShadedDrawing.drawLineAtAngle(gfx, out_r, scale_min_r,
						curr_angle, img_width / 2, (int) R);
			}
			...
			for (curr = min_value; curr <= max_value; curr += major_step) {
				curr_angle = 90.0 + (double) inner_angle / 2
						- ((double) inner_angle / (max_value - min_value))
						* (curr - min_value);
				gfx.setForegroundColor(CustomMediaFactory.getInstance()
						.getColor(scale_color));
				ShadedDrawing.drawLineAtAngle(gfx, out_r, scale_maj_r,
						curr_angle, img_width / 2, (int) R);
				...
				TextPainter.drawRotatedText(gfx, val, 90.0 - curr_angle,
						...
						(int) R - (int) (sine(curr_angle) * text_r),
						TextPainter.CENTER);
			}
		}
		 ...
		 * 
		 * WARNING: if the starting and ending point of an arc are close
		 * together, the arc may not be drawn at all due to rounding errors (the
		 * angles given must be integer). For example if start=-89 and
		 * angle=359, the endings are only a pixel apart, if the radius is less
		 * than about 54 pixels. The ending is further to the left (drawing is
		 * counter-clockwise), meaning that the arc is only 1 pixel long,
		...
		private void drawFrame(Graphics gfx) {
			...
			ShadedDrawing.drawLineAtAngle(gfx, inn_r, out_r, 90 - angle / 2,
					img_width / 2, (int) R);
			AntialiasingUtil.getInstance().disableAntialiasing(gfx);
