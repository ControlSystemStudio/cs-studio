		...
		private void execute(String newText) {
			...
			if (inputTextProperty != null) {
				if (_executionMode==ExecutionMode.RUN_MODE) {
					// In RUN mode set the manual value, because the connected channel sets the property value.
	...
	private class LabelDirectEditPolicy extends DirectEditPolicy {
		...
		protected Command getDirectEditCommand(final DirectEditRequest request) {
			DirectEditCommand command = new DirectEditCommand((String) request
					.getCellEditor().getValue(), getExecutionMode());
			return command;
		}
	...
	protected void registerPropertyChangeHandlers() {
		...
		IWidgetPropertyChangeHandler textHandler = new IWidgetPropertyChangeHandler() {
			public boolean handleChange(final Object oldValue,
				...
				label
						.setTextValue(determineLabel(TextInputModel.PROP_INPUT_TEXT));
				return true;
			}
		};
		...
		setPropertyChangeHandler(TextInputModel.PROP_TEXT_ALIGNMENT,
				alignmentHandler);
		...
		IWidgetPropertyChangeHandler precisionHandler = new IWidgetPropertyChangeHandler() {
			public boolean handleChange(final Object oldValue,
				...
				RefreshableLabelFigure label = (RefreshableLabelFigure) refreshableFigure;
				label
						.setTextValue(determineLabel(TextInputModel.PROP_PRECISION));
				return true;
			}
		};
		setPropertyChangeHandler(TextInputModel.PROP_PRECISION,
				precisionHandler);
		...
		IWidgetPropertyChangeHandler pvHandler = new IWidgetPropertyChangeHandler() {
			public boolean handleChange(final Object oldValue,
				...
				RefreshableLabelFigure label = (RefreshableLabelFigure) refreshableFigure;
				label
						.setTextValue(determineLabel(TextInputModel.PROP_PRIMARY_PV));
				return true;
			}
		};
			...
			if (updatedPropertyId == null
				...
				try {
					try {
						...
						numberFormat.setMaximumFractionDigits(model
								.getPrecision());
						numberFormat.setMinimumFractionDigits(model
								.getPrecision());
