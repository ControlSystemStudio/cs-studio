 ...
 * 
 */
public final class RefreshableMeterFigure extends Shape implements ICrossedFigure {
    /**
    ...
    private void calculateRadii(final int offset) {
        ...
        if (_angle > 180) {
            ...
            if ((int) (inn_r - inn_r * sine(90.0 - _angle / 2)) < values_font.getFontData()[0]
                    .getHeight() * 2) {
                calculateRadii(values_font.getFontData()[0].getHeight() * 2);
            } else {
                _valY = _topDelta
                        + (int) (R - inn_r + (inn_r - inn_r * sine(90.0 - _angle / 2)) / 2);
            }
        }
    }
    ...
    class _MeterBackground extends Shape {
         ...
         */
        protected void fillShape(final Graphics gfx) {
            double upp_angle = ((double) _innerAngle / (_maxValue - _minValue))
            ...
            gfx.fillArc((int) (_imgWidth / 2 - out_r),
                        (int) (R - out_r) + _topDelta,
                        (int) (out_r * 2.0),
                        (int) (out_r * 2.0),
                        (int) Math.round(lo_angle - upp_angle),
                        (int) Math.round(upp_angle));
            // lo area
            lo_angle -= upp_angle;
            ...
            // background color
            gfx.setBackgroundColor(getBackgroundColor());
            gfx.fillArc((int) (_imgWidth / 2 - inn_r),
                        (int) (R - inn_r) + _topDelta,
                        (int) (inn_r * 2.0),
                        (int) (inn_r * 2.0),
        ...
        private void drawScale(final Graphics gfx) {
            ...
            for (curr = _minValue + _minorStep; curr <= _maxValue - _minorStep; curr += _minorStep) {
                curr_angle = 90.0 + (double) _innerAngle / 2
                        - ((double) _innerAngle / (_maxValue - _minValue)) * (curr - _minValue);
            ...
            for (curr = _minValue; curr <= _maxValue; curr += _majorStep) {
                ...
                gfx.setForegroundColor(getForegroundColor());
                TextPainter.drawRotatedText(gfx, val, 90.0 - curr_angle, _imgWidth / 2
                        + (int) (cosine(curr_angle) * text_r), (int) R
                        - (int) (sine(curr_angle) * text_r) + _topDelta, TextPainter.CENTER);
            }
        }
        ...
        private void drawFrame(final Graphics gfx) {
            ...
            gfx.setLineWidth(1);
            // outer arc
            gfx.drawArc((int) (_imgWidth / 2 - out_r),
                        (int) (R - out_r) + _topDelta,
                        (int) out_r * 2,
                        (int) out_r * 2,
                        (int) Math.round(90.0 - _angle / 2),
                        _angle);
            // inner arc
            gfx.drawArc((int) (_imgWidth / 2 - inn_r),
                        (int) (R - inn_r) + _topDelta,
                        (int) inn_r * 2,
                        (int) inn_r * 2,
                        (int) Math.round(90.0 - _angle / 2),
            ...
            ShadedDrawing.drawLineAtAngle(gfx,
                                          ...
                                          _imgWidth / 2,
                                          (int) R + _topDelta);
            ShadedDrawing.drawLineAtAngle(gfx, inn_r, R, 90 - _angle / 2, _imgWidth / 2, (int) R
                    + _topDelta);
        }
    }
    ...
    class _MeterNeedle extends Shape {
        ...
        protected void outlineShape(final Graphics gfx) {
            ...
            double currAngle = 90.0 + (double) _innerAngle / 2
                    - ((double) _innerAngle / (_maxValue - _minValue)) * (_value - _minValue);
            ShadedDrawing.drawLineAtAngle(gfx, inn_r, out_r, currAngle, _imgWidth / 2, (int) R
                    + _topDelta);
    ...
    private final class MeterBorder extends AbstractBorder {
        ...
        public void paint(final IFigure figure, final Graphics graphics, final Insets insets) {
            ...
            // outer arc
            graphics.drawArc((int) (_imgWidth / 2 - inn_r) + bounds.x, (int) (R - inn_r)
                    + _topDelta + bounds.y, (int) inn_r * 2, (int) inn_r * 2, (int) Math
                    .round(90.0 - _angle / 2), _angle);
            // inner arc
            graphics.drawArc(_imgWidth / 2 - (int) R + bounds.x,
                             _topDelta + bounds.y,
                             (int) (2.0 * R),
                             (int) (2.0 * R),
                             (int) Math.round(90.0 - _angle / 2),
                             _angle);
            // left and right lines
            ShadedDrawing.drawLineAtAngle(graphics, inn_r, R, 90 + _angle / 2, _imgWidth / 2
                    + bounds.x, (int) R + _topDelta + bounds.y);
            ShadedDrawing.drawLineAtAngle(graphics, inn_r, R, 90 - _angle / 2, _imgWidth / 2
                    + bounds.x, (int) R + _topDelta + bounds.y);
        }
    }
