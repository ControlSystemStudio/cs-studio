...
public class ArticleGuiForm extends AbstractGuiFormTemplate<Article> implements PropertyChangeListener {
	...
	// the InstalledView.
	// itself.
	protected void beforeExecuteSearch(List<SearchTerm> searchTerms) {
	...
	private void createPart(final Composite composite) {
		...
		wf.text(composite, "beschreibung").multiLine().isJoined().label("Note:", "ay top")
		...
		if (isSearchMode()) {
			...
			getEditorInput().processData(new Func1Void<Some<Article>>() {
				@Override
				public void apply(Some<Article> article) {
					BigDecimal gruppeArtikel = article.get().getGruppeArtikel();
					articleDescriptionId = article.get().getArticleDescription().getId();
	...
	protected void createAdditionalSearchResultButtons(Composite composite, TableViewer viewer) {
		...
		MigLayout ml = new MigLayout("ins 0", "[150,fill][150, fill][150, fill]", "[]");
		buttonComposite.setLayout(ml);
		wf.button(buttonComposite, "assignId").text("Show article history").listener(new SimpleSelectionListener() {
			@Override
			public void widgetSelected(SelectionEvent e) {
				Dialogs.message("Note", "Not implemented");
			}
		}).build();
		wf.button(buttonComposite, "contains").text("Show contains").listener(new SimpleSelectionListener() {
			@Override
			public void widgetSelected(SelectionEvent e) {
				Dialogs.message("Note", "Not implemented");
			}
		}).build();
		wf.button(buttonComposite, "assignId").text("Show is contained in").listener(new SimpleSelectionListener() {
			@Override
			public void widgetSelected(SelectionEvent e) {
				Dialogs.message("Note", "Not implemented");
			}
	...
	private void createItemSelectionCombo(final Composite composite, final GenericEditorInput<Article> editorInput) {
		editorInput.processData(new Func1Void<Some<Article>>() {
			@Override
			public void apply(Some<Article> article) {
				List<Article> articles = articleService.findAllArticleInGroup(article.get().getGruppeArtikel());
				articlesInGroup = new WritableList(articles, articles.getClass());
				Combo itemSelectionCombo = wf.combo(composite, ITEM_COMBO.getName()).label("Item Position:")
							.noBinding().data(articlesInGroup, new Property("index")).select(article.get())
							.hint("growx, wrap, gaptop 7, gapbottom 7").build();
						...
						public void widgetSelected(SelectionEvent e) {
							...
							if (!orderPos.isEmpty()) {
								...
								if (anzahlBestellt != null) {
									...
									propertyChange(new PropertyChangeEvent(ArticleGuiForm.this,
												EVENT_SOURCE_ARTICLE_NEW, article, article));
								}
							}
						}
					}).build();
