...
public class InstalledView extends AbstractSubView<ArticleInstalled> {
	...
	public void updateAfterArticleSelection(IStructuredSelection selection) {
		...
		getEditorInput().get().processData(new Func1Void<Some<ArticleInstalled>>() {
			@Override
			public void apply(Some<ArticleInstalled> articleInstalled) {
				if (articleInstalled.hasValue()) {
					articleInstalled.get().setEingebautInArtikel(artikelDatenId);
				}
			}
		});
	}
	...
	private void displayArticleData(Article article) {
		...
		if (!allArticleInstalled.isEmpty()) {
			ArticleInstalled articleInstalled = allArticleInstalled.get(allArticleInstalled.size() - 1);
			wf.setText(P("locationDetails"), articleInstalled.getLocationDetails());
			wf.setText(P("gebaeude"), articleInstalled.getGebaeude());
			wf.setText(P("raum"), articleInstalled.getRaum());
		}
	}
	...
	public Some<Composite> build(CrudController<Article> crudController, ArticleInstalled articleInstalled,
		...
		wf.text(compositeArticleData, "beschreibung").multiLine().isJoined().label("Note:", "ay top")
