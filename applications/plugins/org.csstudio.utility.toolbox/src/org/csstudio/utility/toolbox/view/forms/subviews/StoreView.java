...
public class StoreView extends AbstractSubView<ArticleInStore> {
	...
	private class LagerSelectionLister extends SimpleSelectionListener {
		...
		public void widgetSelected(SelectionEvent e) {
			getEditorInput().get().processData(new Func1Void<Some<ArticleInStore>>() {
				@Override
				public void apply(Some<ArticleInStore> articleInStore) {
	...
	public Some<Composite> build(CrudController<Article> crudController, ArticleInStore articleInStore,
		...
		wf.combo(composite, "inLagerDurch").label("Put in stock by:").data(logUserService.findAll()).hint("wrap")
					.build();
		...
		getEditorInput().get().processData(new Func1Void<Some<ArticleInStore>>() {
			...
			public void apply(Some<ArticleInStore> articleInStore) {
				if (articleInStore.hasValue()) {
					wf.setText(P("lagerArtikelId"), articleInStore.get().getLagerArtikelId());
