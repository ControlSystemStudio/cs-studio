...
public final class ActionDataCellEditor extends AbstractDialogCellEditor {
	...
	protected void openDialog(final Shell parentShell, final String dialogTitle) {
		ActionDataDialog dialog = new ActionDataDialog(parentShell,dialogTitle,"Set the attributes for the action");
	...
	protected void doSetValue(final Object value) {
		...
		if (value==null || !(value instanceof ActionData)) {
			...
			_actionData = new ActionData();
			for (WidgetAction action : _originalActionData.getWidgetActions()) {
	...
	private final class ActionDataDialog extends Dialog {
	    ...
	    /**
	     * The List for the {@link WidgetAction}s.
	     */
	    ...
	    /**
	     * The Action to add a new {@link WidgetAction}.
	     */
	    private Action _addAction;
	    /**
	     * The Action to copy the selected {@link WidgetAction}.
	     */
	    private Action _copyAction;
	    /**
	     * The Action to delete the selected {@link WidgetAction}.
	     */
	    ...
	    /**
	     * The currently opened CellEditor.
	     * If no one is opened this value is null.
	     */
	     ...
	     */
		public ActionDataDialog(final Shell parentShell, final String dialogTitle,
	    ...
	    protected Control createDialogArea(final Composite parent) {
	        ...
	        _actionViewer.setInput(_actionData.getWidgetActions().toArray(new WidgetAction[0]));
		...
		/**
		 * Creates the popup-menu for adding a {@link WidgetAction}.
		 * @param control The {@link Control} for the menu
		 * @param withRemoveAction Indicates if an action to remove a {@link WidgetAction} should be added
		...
		private void createActions() {
			...
			_copyAction = new Action() {
				@Override
				public void run() {
					IStructuredSelection selection = (IStructuredSelection) _actionViewer.getSelection();
					if (!selection.isEmpty() && selection.getFirstElement() instanceof WidgetAction) {
						_actionData.addAction(((WidgetAction)selection.getFirstElement()).makeCopy());
						refreshActionViewer((WidgetAction)selection.getFirstElement());
					}
				}
			};
			...
			_removeAction = new Action() {
				...
				public void run() {
					IStructuredSelection selection = (IStructuredSelection) _actionViewer.getSelection();
					if (!selection.isEmpty() && selection.getFirstElement() instanceof WidgetAction) {
						_actionData.removeAction((WidgetAction)selection.getFirstElement());
			...
			_removeAction.setToolTipText("Removes the selected Action from the list");
		...
		/**
		 * Calculates the input for the table
		 * depending on the type of the {@link ActionData}.
		 * @param action The {@link WidgetAction} for the input
		 */
		private void calculatePropertyInput(final WidgetAction action) {
			...
			if (action!=null) {
		        ...
		        for (String propertyKey : propertyKeys) {
		        	...
		        	IPropertyDescriptorFactory factory = PropertyDescriptorFactoryService.getInstance().getPropertyDescriptorFactory(property.getPropertyType());
		        	descriptors[i] = factory.createPropertyDescriptor(propertyKey, property);
		        	i++;
		        }	
			}
		 ...
		 * @param parent The parent for the label
		 ...
		 * @param parent The parent for the table
		...
		private void refreshActions() {
			...
			IStructuredSelection selection = (IStructuredSelection) _actionViewer.getSelection();
			if (!selection.isEmpty() && selection.getFirstElement() instanceof WidgetAction && _openedCellEditor==null) {
				_removeAction.setEnabled(true);
				_copyAction.setEnabled(true);
				calculatePropertyInput((WidgetAction)selection.getFirstElement());
			} else {
				_removeAction.setEnabled(false);
				_copyAction.setEnabled(false);
			}
		...
		private TableViewer createPropertyTableViewer(final Composite parent) {
			final Table table = new Table(parent, SWT.FULL_SELECTION | SWT.H_SCROLL | SWT.V_SCROLL | SWT.BORDER);
			...
			viewer.setColumnProperties(new String[] {"first", "Property", "Value"});
			...
			viewer.addDropSupport(DND.DROP_COPY | DND.DROP_MOVE, 
					...
					for (TransferData transfer : event.dataTypes) {
						if (TextTransfer.getInstance().isSupportedType(transfer)) {
							if (event.item instanceof TableItem) {
								...
								IPropertyDescriptor desc = (IPropertyDescriptor) item.getData();
				...
				public void drop(final DropTargetEvent event) {
					if (event.data instanceof String) {
						IPropertyDescriptor desc = (IPropertyDescriptor) ((TableItem)event.item).getData();
		 ...
		 */
		private WidgetProperty getWidgetProperty(final IPropertyDescriptor descriptor) {
			IStructuredSelection selection = (IStructuredSelection) _actionViewer.getSelection();
			if (!selection.isEmpty() && selection.getFirstElement() instanceof WidgetAction) {
				WidgetAction action = (WidgetAction) selection.getFirstElement();
				for (String propertyKey : action.getPropertyKeys()) {
					if (descriptor.getId().equals(propertyKey)) {
						return action.getProperty(propertyKey);
					}
				}
			}
			return null;
		...
		/**
		 * Refreshes the viewer for the {@link WidgetAction}s.
		 * @param action The action to select (may be null)
		 */
		private void refreshActionViewer(final WidgetAction action) {
			new CheckedUiRunnable() {
				@Override
				protected void doRunInUi() {
					WidgetAction[] input = _actionData.getWidgetActions().toArray(new WidgetAction[0]);
					_actionViewer.setInput(input);
					_actionViewer.refresh();
					if (action==null) {
						_actionViewer.setSelection(StructuredSelection.EMPTY);
					} else {
						_actionViewer.setSelection(new StructuredSelection(action));
					}
		...
		private final class CustomEditingSupport extends EditingSupport {
			 ...
			 */
			private CustomEditingSupport(final ColumnViewer viewer, final Table table) {
				super(viewer);
				_table = table;
			}
			...
			protected CellEditor getCellEditor(final Object element) {
				if (!_cellEditors.containsKey(element)) {
					...
					IPropertyDescriptor descriptor = (IPropertyDescriptor) selection.getFirstElement();
					if (descriptor!=null) {
						CellEditor editor = descriptor.createPropertyEditor(_table);
						if (editor!=null) {
			...
			protected void setValue(final Object element, final Object value) {
				if (element instanceof IPropertyDescriptor) {
					...
					if (prop!=null) {
						...
						_propertyViewer.refresh();
						refreshActionViewer((WidgetAction) ((IStructuredSelection)_actionViewer.getSelection()).getFirstElement());
					}
				}
			}
		}
		...
		/**
		 * An {@link Action}, which adds a new {@link WidgetAction} of the given {@link ActionType}.
		 * @author Kai Meyer
		 *
		 */
		private final class TypeAction extends Action {
			...
			/**
			 * Constructor.
			 * @param type The {@link ActionType} for the action.
			 */
			public TypeAction(final ActionType type) {
				_type = type;
				this.setText("Add "+_type.getTitle());
				WidgetAction widgetAction = _type.getActionFactory().createWidgetAction();
			...
			public String getColumnText(final Object element, final int columnIndex) {
				if (element instanceof IPropertyDescriptor) {
					...
					if (columnIndex==1) {
						...
						if (descriptor.getLabelProvider()!=null && widgetProperty!=null) {
							return descriptor.getLabelProvider().getText(widgetProperty.getPropertyValue());
						}
					}
				}	
				if (element!=null) {
