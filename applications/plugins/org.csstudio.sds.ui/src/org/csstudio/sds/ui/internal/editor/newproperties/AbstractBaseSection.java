...
public abstract class AbstractBaseSection<E extends WidgetProperty> extends AbstractPropertySection implements IPropertyChangeListener {
	...
	public final void createControls(final Composite parent, TabbedPropertySheetPage aTabbedPropertySheetPage) {
		...
		GridDataFactory.fillDefaults().hint(30, SWT.DEFAULT).align(SWT.BEGINNING, SWT.BEGINNING).applyTo(nameHyperlink);
		Image image = CustomMediaFactory.getInstance().getImageFromPlugin(SdsUiPlugin.PLUGIN_ID, "icons/info.png");
		...
		editSettingsButton.setImage(CustomMediaFactory.getInstance().getImageFromPlugin(SdsUiPlugin.PLUGIN_ID, "icons/dynamics_add.gif"));
		editSettingsButton.setToolTipText("Edit channel configuration");
		GridDataFactory.swtDefaults().align(SWT.BEGINNING, SWT.BEGINNING).applyTo(editSettingsButton);
		editSettingsButton.addHyperlinkListener(new HyperlinkAdapter() {
			@Override
			public void linkActivated(HyperlinkEvent e) {
				WidgetProperty p = getMainWidgetProperty();
				IPropertyDescriptorFactory desriptorFactory = PropertyDescriptorFactoryService.getInstance().getPropertyDescriptorFactory(
						p.getPropertyType());
				IPropertyDescriptor descriptor = desriptorFactory.createPropertyDescriptor(p.getId(), p);

				Map<String, String> allInheritedAliases = mainWidgetProperty.getWidgetModel().getAllInheritedAliases();
				
				// .. open the dynamics wizard
				DynamicAspectsWizard wizard = new DynamicAspectsWizard(mainWidgetProperty.getDynamicsDescriptor(), allInheritedAliases,
						descriptor, mainWidgetProperty.getPropertyValue());

				if (wizard != null) {
					if (Window.OK == ModalWizardDialog.open(Display.getCurrent().getActiveShell(), wizard)) {
		...
		removeSettingsButton.setImage(CustomMediaFactory.getInstance().getImageFromPlugin(SdsUiPlugin.PLUGIN_ID, "icons/dynamics_remove.gif"));
		GridDataFactory.swtDefaults().align(SWT.BEGINNING, SWT.BEGINNING).applyTo(removeSettingsButton);
	 ...
	 */
	protected abstract void doCreateControls(final Composite parent, TabbedPropertySheetPage aTabbedPropertySheetPage);
	...
	public void setInput(IWorkbenchPart part, ISelection selection) {
		...
		mainWidgetProperty = selectedWidget != null ? (E) selectedWidget.getPropertyInternal(propertyId) : null;
	...
	protected void applyPropertyChange(Object newValue) {
		if (newValue != null && mainWidgetProperty != null
				&& !mainWidgetProperty.getPropertyValue().equals(mainWidgetProperty.checkValue(newValue))) {
	...
	protected void applyDynamics(DynamicsDescriptor newDynamicsDescriptor) {
		...
		if (selectedWidgets != null) {
			for (AbstractWidgetModel widget : selectedWidgets) {
				assert widget.hasProperty(propertyId);
				chain.add(new ChangeDynamicsCommand(widget.getPropertyInternal(propertyId), newDynamicsDescriptor != null ? newDynamicsDescriptor
						.clone() : null));
			}
		}
