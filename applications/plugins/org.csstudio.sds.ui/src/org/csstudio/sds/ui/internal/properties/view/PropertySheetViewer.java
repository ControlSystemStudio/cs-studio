...
final class PropertySheetViewer extends Viewer {
	 ...
	 */
	private ListenerList _activationListeners = new ListenerList();
	...
	public PropertySheetViewer(final Composite parent) {
		_tree = new Tree(parent, SWT.FULL_SELECTION | SWT.SINGLE
				| SWT.HIDE_SELECTION);
		...
		TreeViewer viewer = new TreeViewer(_tree);

		ProcessVariableExchangeUtil.addProcessVariableAddressDropSupport(viewer
				.getControl(), DND.DROP_COPY | DND.DROP_NONE,
				new IProcessVariableAdressReceiver() {
					public void receive(IProcessVariableAddress[] pvs,
						...
						if (event.item != null
							...
							IPropertySheetEntry entry = (IPropertySheetEntry) event.item
									.getData();
							DynamicsDescriptor[] descriptors = entry
									.getDynamicsDescriptors();
							DynamicsDescriptor dynamicsDescriptor;
							if (descriptors == null || descriptors.length == 0
									|| descriptors[0] == null) {
								dynamicsDescriptor = new DynamicsDescriptor();
							} else {
								dynamicsDescriptor = entry
										.getDynamicsDescriptors()[0].clone();
							}
							...
							DynamicAspectsWizard wizard = (DynamicAspectsWizard) entry
	 ...
	 */
	public void addWidgetSelectionListener(
			final IWidgetSelectionListener listener) {
	...
	private void addColumns() {
		...
		// create the columns
		TreeColumn[] columns = _tree.getColumns();
		for (int i = 0; i < _columnLabels.length; i++) {
			String string = _columnLabels[i];
			if (string != null) {
				TreeColumn column;
				if (i < columns.length) {
					column = columns[i];
		...
		_tree.addControlListener(new ControlAdapter() {
			...
			public void controlResized(final ControlEvent e) {
				Rectangle area = _tree.getClientArea();
	...
	private void applyEditorValue() {
		TreeItem treeItem = _treeEditor.getItem();
	 ...
	 */
	private void createItem(final Object node, final Widget parent,
			final int index) {
		// create the item
		TreeItem item;
		...
		if (node instanceof IPropertySheetEntry) {
			((IPropertySheetEntry) node)
					.addPropertySheetEntryListener(_entryListener);
		}
	...
	private void fireCellEditorActivated(final CellEditor activatedCellEditor) {
		Object[] listeners = _activationListeners.getListeners();
	...
	private List getChildren(final IPropertySheetEntry entry) {
		...
		if (entry == _rootEntry && _isShowingCategories) {
			if (_categories.length > 1
					|| (_categories.length == 1 && !_categories[0]
	 ...
	 *         objects for which the viewer is currently showing properties. It
	 *         returns an <code>Object[]</code> or <code>null</code>.
	 */
	@Override
	 ...
	 *         <p>
	 *         Note that this method only includes
	 *         <code>IPropertySheetEntry</code> in the selection (no
	 *         categories).
	 *         </p>
	 */
	@SuppressWarnings("unchecked")
		...
		_tree.addMouseListener(new MouseAdapter() {
			...
			public void mouseDown(final MouseEvent event) {
				...
				if (event.button == 1 && event.stateMask == 0) {
					...
					TreeItem item = _tree.getItem(pt);
					if (item != null) {
						handleSelect(item);
					}
				}
	...
	private void removeItem(final TreeItem item) {
		...
		if (data instanceof IPropertySheetEntry) {
			((IPropertySheetEntry) data)
					.removePropertySheetEntryListener(_entryListener);
		}
		item.setData(null);
		item.dispose();
	}
	...
	public void setInput(final Object newInput) {
		...
		Object[] objects = (Object[]) newInput;
		...
		if (_input != null
			...
			for (IWidgetSelectionListener listener : _widgetListener) {
	...
	private void updateCategories() {
		...
		List childEntries = getFilteredEntries(_rootEntry.getChildEntries());
		...
		Map categoryCache = new HashMap(_categories.length * 2 + 1);
		for (int i = 0; i < _categories.length; i++) {
			_categories[i].removeAllEntries();
			categoryCache.put(_categories[i].getCategoryName(), _categories[i]);
		}

		// create a list of categories to get rid of
		List categoriesToRemove = new ArrayList(Arrays.asList(_categories));
	 ...
	 *            the tree item for the given entry
	 */
	protected void updateEntry(final IPropertySheetEntry entry,
			final TreeItem item) {
		// ensure that backpointer is correct
		item.setData(entry);
		...
		if (entry.isDynamicallySampled()) {
			item.setImage(0, CustomMediaFactory.getInstance()
