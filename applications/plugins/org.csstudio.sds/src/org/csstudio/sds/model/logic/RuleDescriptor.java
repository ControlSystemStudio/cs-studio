...
public final class RuleDescriptor implements IAdaptable {
	...
	public RuleDescriptor(final String ruleId, final String description,
			final String[] parameterDescriptions, final Class[] parameterTypes, 
			final Class returnType, final IRule rule, final boolean isScriptedRule) {
		assert ruleId != null;
		...
		if (parameterTypes==null) {
			_parameterTypes = new Class[_parameterDescriptions.length];
			for (int i=0;i<_parameterTypes.length;i++) {
				_parameterTypes[i] = Object.class;
			}
		} else {
			_parameterTypes = parameterTypes;
		}
		if (_parameterDescriptions.length>_parameterTypes.length) {
			CentralLogger.getInstance().warn(this, "_parameterDescriptions.length>_parameterTypes.length");
			_parameterTypes = new Class[_parameterDescriptions.length];
	...
	@SuppressWarnings("unchecked")
	public RuleDescriptor(final String ruleId, final String description,
			final String[] parameterDescriptions, final Class returnType, final IRule rule,
			final boolean isScriptedRule) {
		this(ruleId, description, parameterDescriptions, null, returnType, rule, isScriptedRule);
	}
	...
	/**
	 * Return the types of the expected parameters.
	 * 
	 * @return The textual descriptions of the expected parameters.
	 */
	@SuppressWarnings("unchecked")
