...
public final class DisplayModelInputStream extends ByteArrayInputStream {
	...
	private Element createDisplayTag(final DisplayModel displayModel) {
		...
		result.setAttribute(XmlConstants.XML_ATTRIBUTE_MODEL_VERSION, "1.0"); //$NON-NLS-1$

		for (String propertyId : displayModel.getPropertyNames()) {
			result.addContent(createPropertyTag(propertyId, displayModel
	...
	private Element createModelTag(final AbstractWidgetModel widgetModel) {
		...
		for (String propertyId : widgetModel.getPropertyNames()) {
			result.addContent(createPropertyTag(propertyId, widgetModel
					.getProperty(propertyId)));
		}
	 ...
	 *         <code>WidgetProperty</code>.
	 */
	private Element createPropertyTag(final String propertyId,
		...
		result.setAttribute(XmlConstants.XML_ATTRIBUTE_PROPERTY_ID, propertyId);
	...
	private Element createParameterElement(final String elementName,
		...
		result.setAttribute(XmlConstants.XML_ATTRIBUTE_CHANNEL_TYPE, parameter
				.getType().getName());
	...
	/**
	 * Set the XML Header for the Stream. The default is
	 * "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n"
	 * 
	 * @param header The XML Header.
	 */
    public void setXMLHeader(final String header) {
        _xmlHeader = header;
    }
}
