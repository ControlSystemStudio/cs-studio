...
public final class SaxDisplayModelHandler extends DefaultHandler {
	...
	public void startElement(final String uri, final String localName,
		...
		if (!_modelPropertiesLoaded
				&& XmlConstants.XML_ELEMENT_WIDGET.equals(element.getName())) {
	 ...
	 */
	private void parseLayer(final Element element,
			final ContainerModel container) throws SAXException {
		final String layerName = element
				.getAttributeValue(XmlConstants.XML_LAYER_NAME);
		boolean visibility = true;
		try {
			visibility = element
					.getAttribute(XmlConstants.XML_LAYER_VISIBILITY)
					.getBooleanValue();
		} catch (DataConversionException e) {
			throw new SAXException("Unable to read <"
					+ XmlConstants.XML_LAYER_VISIBILITY
					+ "> property from layer element", e);
		}
		final boolean isVisible = visibility;
		int index = -1;
		try {
			index = element.getAttribute(XmlConstants.XML_LAYER_INDEX)
					.getIntValue();
		} catch (DataConversionException e) {
			throw new SAXException("Unable to read <"
					+ XmlConstants.XML_LAYER_INDEX
					+ "> property from layer element", e);
		}
		final int layerIndex = index;

		Layer layer = new Layer(layerName, layerName);
	 ...
	 *             if an error occurred during parsing.
	 */
	private AbstractWidgetModel parseWidgetModel(final Element element)
			throws SAXException {
	 ...
	 *             if an error occurred during parsing.
	 */
	private void fillWidgetModel(final AbstractWidgetModel widgetModel,
			final Element widgetElement) throws SAXException {
		...
		// 3. properties
		List propertiesList = widgetElement
				.getChildren(XmlConstants.XML_ELEMENT_PROPERTY);
	 ...
	 *             if an error occurred during parsing.
	 */
	private void setProperty(final AbstractWidgetModel widgetModel,
		...
		if (persistenceHandler != null) {
			...
			Object propertyValue = parsePropertyValue(persistenceHandler,
					propertyElement);
			...
			if (dynamicsDescriptorElement != null) {
				DynamicsDescriptor dynamicsDescriptor = parseDynamicsDescriptor(
						persistenceHandler, dynamicsDescriptorElement);

				widgetModel.setDynamicsDescriptor(propertyId,
						dynamicsDescriptor);
			}
		}
	}
	...
	private DynamicsDescriptor parseDynamicsDescriptor(
		...
		// connection states
		result.setConnectionStateDependentPropertyValues(parseConnectionStates(
				persistenceHandler, dynamicsDescriptorElement));
	 ...
	 */
	private HashMap<ConnectionState, Object> parseConnectionStates(
			final AbstractPropertyPersistenceHandler persistenceHandler,
			final Element dynamicsDescriptorElement) throws SAXException {
	 ...
	 *             if a connection state could not be parsed.
	 */
	private HashMap<DynamicValueState, Object> parseDynamicValueStates(
			final AbstractPropertyPersistenceHandler persistenceHandler,
			final Element dynamicsDescriptorElement) throws SAXException {
			...
			for (Object o : dynamicValueStateElements) {
				...
				try {
					state = DynamicValueState
							.valueOf(dynamicValueStateElement
									.getAttributeValue(XmlConstants.XML_ATTRIBUTE_STATE));
				} catch (Exception e) {
					throw new SAXException(
							"Could not parse dynamic value state <"
									+ dynamicValueStateElement
											.getAttributeValue(XmlConstants.XML_ATTRIBUTE_STATE)
									+ ">", e);
				}
				result.put(state, persistenceHandler
						.readProperty(dynamicValueStateElement));
			}
		}
	 ...
	 *             if an error occurred during parsing.
	 */
	private ParameterDescriptor parseChannel(final Element channelElement)
			throws SAXException {
	 ...
	 */
	private Element createElement(final String qName,
			final Attributes attributes) {
		Element result = new Element(qName);

		if (attributes != null) {
			for (int i = 0; i < attributes.getLength(); i++) {
				result.setAttribute(attributes.getQName(i), attributes
						.getValue(i));
			}
		}
