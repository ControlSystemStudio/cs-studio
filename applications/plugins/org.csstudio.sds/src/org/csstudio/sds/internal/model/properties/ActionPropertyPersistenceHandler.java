...
public final class ActionPropertyPersistenceHandler extends
	...
	public void writeProperty(final Element domElement,
		...
		for (WidgetAction action : actionData.getWidgetActions()) {
			...
			for (String propertyKey : action.getPropertyKeys()) {
				...
				AbstractPropertyPersistenceHandler persistenceHandler = PropertyPersistenceHandlerRegistry.getInstance().getPersistenceHandler(property.getPropertyType());
				Element propertyTag = createPropertyTag(propertyKey, property);
				persistenceHandler.writeProperty(propertyTag, property.getPropertyValue());
	...
	private Element createPropertyTag(final String propertyKey,
		...
		result.setAttribute(XmlConstants.XML_ATTRIBUTE_PROPERTY_ID, propertyKey);
	 ...
	 *         <code>WidgetProperty</code>.
	 */
	private Element createActionTag(final WidgetAction elementAction) {
		Element result = new Element(XML_ACTION_ATTRIBUT);
		result.setAttribute(XML_ATTRIBUT_ACTION_TYPE,
				elementAction.getType().toString());
		return result;
	}
	...
	public Object readProperty(final Element domElement) {
		...
		for (Object action : actionDataElement.getChildren(XML_ACTION_ATTRIBUT)) {
			...
			String actionType = actionElement.getAttributeValue(XML_ATTRIBUT_ACTION_TYPE);
			WidgetAction widgetAction = ActionType.valueOf(actionType).getActionFactory().createWidgetAction();
			for (Object property : actionElement.getChildren(XML_PROPERTY_ATTRIBUT)) {
				Element propertyElement = (Element) property;
				String propertyKey = propertyElement.getAttributeValue(XmlConstants.XML_ATTRIBUTE_PROPERTY_ID);
				String propertyType = propertyElement.getAttributeValue(XmlConstants.XML_ATTRIBUTE_PROPERTY_TYPE);
				if (propertyKey!=null && propertyType!=null) {
					...
					Object propertyValue = persistenceHandler.readProperty(propertyElement);
					widgetAction.getProperty(propertyKey).setPropertyValue(propertyValue);	
				}
			}
