...
{    
    public static String[] getPhoneNumbers(String groupNames)
    {
        ...
        try
        {
            ...
            while(token.hasMoreTokens())
            {
                ...
                Vector<UserTObject> users = UserGroupUserDAO.selectByGroupAndState(connection, groupId, 1);
                if(users.isEmpty() == false)
                {
                    for(UserTObject u : users)
                    {
    ...
    public static String[] getEMailAddresses(String groupNames)
    {
        StringTokenizer token = null;
        Connection connection = null;
        ArrayList<UserGroupKey> groupKey = null;
