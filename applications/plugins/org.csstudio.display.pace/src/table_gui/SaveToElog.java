...
*/
public class SaveToElog extends TitleAreaDialog {
/**
...
public SaveToElog(final Composite s, final Model m, final Table table, final int column, final String value) {
...
private boolean CRpressed(Composite shell, String summary, String inval)
{
   ...
   try
   {
      ...
      int selections[] = t.getSelectionIndices();
      ...
      {  
         ...
         {
            if(j == col) 
              model.getRow(selections[i]).getCell(j).writeMultiValue(s, inval);
         }
      }  
   }
   catch (Exception ex)
...
{    
  ...
  // d.widthHint = 230;
   ...
   String dateTm = "  Date/Time: " + formatter.format(nowDate) + "\n\n";  
    ...
    int selections[] = t.getSelectionIndices();
    // find the edited column
    for (int i = 0; i < selections.length && inVal.length()==0; i++) 
    {   
       rows = model.getRow(selections[i]);
       for(int c = 0; c < model.getNumColumns() && inVal.length()==0;c++)
       {
// This is the code updating the elog screen
           model_cell = rows.getCell(c);
           if( model_cell.hasUserValue() && inVal.length()==0 && c == col) 
           { 
