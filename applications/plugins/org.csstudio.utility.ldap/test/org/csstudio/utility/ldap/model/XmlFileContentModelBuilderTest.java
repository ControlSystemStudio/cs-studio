...
public class XmlFileContentModelBuilderTest {
    ...
    private ContentModel<LdapEpicsControlsObjectClass> _model;

    @BeforeClass
    public static final void buildResourcePath() {
        try {
            final File loc = FileLocator.getBundleFile(LdapActivator.getDefault().getBundle());
    ...
    public void testValid() {
        final XmlFileContentModelBuilder<LdapEpicsControlsObjectClass> builder =
            new XmlFileContentModelBuilder<LdapEpicsControlsObjectClass>(LdapEpicsControlsObjectClass.ROOT,
    ...
    private void testSimpleNamesCache() {
        Set<String> simpleNames = _model.getSimpleNames(LdapEpicsControlsObjectClass.FACILITY);
        Assert.assertEquals(simpleNames.size(), 2);
        simpleNames = _model.getSimpleNames(LdapEpicsControlsObjectClass.COMPONENT);
        Assert.assertEquals(simpleNames.size(), 3);
        simpleNames = _model.getSimpleNames(LdapEpicsControlsObjectClass.IOC);
        Assert.assertEquals(simpleNames.size(), 5);
        simpleNames = _model.getSimpleNames(LdapEpicsControlsObjectClass.RECORD);
    ...
    public void testEmpty() {
        ...
        try {
            ...
            Assert.assertEquals("File " + xmlFilePath + " contains parsing errors.", e.getMessage());
            return;
        }
