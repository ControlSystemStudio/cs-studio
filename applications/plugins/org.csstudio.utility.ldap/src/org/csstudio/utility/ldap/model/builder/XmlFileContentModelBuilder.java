 ...
 */
public class XmlFileContentModelBuilder<T extends Enum<T> & ILdapObjectClass<T>> extends AbstractContentModelBuilder<T> {
     ...
     * Constructor.
     */
    public XmlFileContentModelBuilder(@Nonnull final T objectClassRoot, @Nonnull final String filePath) {
    ...
    private ContentModel<T> createContentModelFromFile(@Nonnull final Document doc)
        ...
        try {
            model = new ContentModel<T>(_objectClassRoot);
    ...
    private void processElement(@Nonnull final ContentModel<T> model,
                                ...
                                @Nonnull final ILdapTreeComponent<T> ldapParent) throws CreateContentModelException {


        final String type = element.getName();
        final T oc = _objectClassRoot.getObjectClassByRdnType(type);
        ...
        if (model.getByTypeAndLdapName(oc, fullName.toString()) == null) {
            ILdapTreeComponent<T> newLdapChild;
            try {
                newLdapChild = new LdapTreeComponent<T>(name,
                        oc,
        ...
        final ILdapTreeComponent<T> ldapComponent =
