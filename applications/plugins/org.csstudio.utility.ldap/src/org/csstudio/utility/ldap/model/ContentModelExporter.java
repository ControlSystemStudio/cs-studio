...
public final class ContentModelExporter {
     ...
     */
    public static <T extends Enum<T> & ILdapObjectClass<T>>
    ...
    @Nonnull
    private static <T extends Enum<T> & ILdapObjectClass<T>>
        ...
        final Element rootElem = new Element(type.getRdnType());
        rootElem.setAttribute("name", type.getRootName());
        ...
        for (final ILdapTreeComponent<T> modelNode : model.getRoot().getDirectChildren()) {
    ...
    private static <T extends Enum<T> & ILdapObjectClass<T>>
        void createDOMElement(@Nonnull final Element parentElem,
                              @Nonnull final ILdapTreeComponent<T> modelNode) {

        // FIXME (bknerr) : once the deprecated esco, ioc components have vanished, the next lines can be removed
        String rdnType = modelNode.getType().getRdnType();
        if ("econ".equals(rdnType) ||
            "esco".equals(rdnType)) {
            rdnType = "ecom";
        }

        final Element newNode = new Element(rdnType);

        newNode.setAttribute("name", modelNode.getName());

        parentElem.addContent(newNode);

        for (final ILdapTreeComponent<T> child : modelNode.getDirectChildren()) {
