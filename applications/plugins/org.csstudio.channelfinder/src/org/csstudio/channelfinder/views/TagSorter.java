...
public class TagSorter extends AbstractColumnViewerSorter {
	...
	public int compare(XmlChannel d1, XmlChannel d2) {
		XmlTag tag1 = getTag(d1, tagName);
		XmlTag tag2 = getTag(d2, tagName);
		if ((tag1 == tag2))
			return 0;
		else if (tag1 == null)
			return -1;
	 ...
	 */
	private XmlTag getTag(XmlChannel channel, String tagName) {
		Iterator<XmlTag> itr = channel.getXmlTags().iterator();
		while (itr.hasNext()) {
			XmlTag item = itr.next();
