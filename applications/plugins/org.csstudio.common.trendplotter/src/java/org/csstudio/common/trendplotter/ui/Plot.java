...
import org.csstudio.common.trendplotter.model.IProcessVariable;
...
//        TODO jhatje: change for new listener in swt plugin
...
//                                         final Range new_range) { // Check that it's not caused by ourself, and a real change
//                if (!plot_changes_timeaxis && !old_range.equals(new_range) && listener != null) {
...
//        };
    ...
    private Axis getYAxis(final int index) {
        ...
        while (axes.size() <= index) {
            ...
            final Axis axis = new Axis(NLS.bind(Messages.Plot_ValueAxisNameFMT, new_axis_index + 1),
                                       true);
...
//        };
    ...
    public void addTrace(final ModelItem item) {
        ...
        final Trace trace = new Trace(item.getDisplayName(), xaxis, yaxis, item.getSamples());
    ...
    private Trace findTrace(final ModelItem item) {
    ...
    public void updateAutoscale() {
        display.asyncExec(new Runnable() {
            @Override
            public void run() {
                for (final Axis yaxis : xygraph.getYAxisList()) {
