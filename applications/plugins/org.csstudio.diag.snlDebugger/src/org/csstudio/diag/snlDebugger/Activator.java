...
public class Activator extends AbstractCssUiPlugin {
	...
	// The plug-in ID
	public static final String PLUGIN_ID = "org.csstudio.diag.snlDebugger"; //$NON-NLS-1$
...
//		super.start(context);
		File defaultFile = new File(getPluginPreferences().getString(SampleService.snlDebugger_XML_FILE_PATH));
		if(!defaultFile.isFile()){
			if(defaultFile.isDirectory()){
				defaultFile = new File(defaultFile,"snlDebugger.xml"); //$NON-NLS-1$
			}
			else{
				defaultFile = new File(getPluginPreferences().getDefaultString(SampleService.snlDebugger_XML_FILE_PATH));
			}
			getPluginPreferences().setValue(SampleService.snlDebugger_XML_FILE_PATH, defaultFile.toString());
