...
public class WorkQueueUnitTest {
	...
	private void fireActions(WorkQueue workQueue, AlarmTreeItem item,
	...
	public void testOverflow() {
		...
		try {
			...
			fireActions(workQueue, pv1, "AA2_", EActionPriority.MINOR, 3);
			...
			fireActions(workQueue, pv2, "AA4_", EActionPriority.MINOR, 3);
			Assert.assertEquals(2, workQueue.countPendingActions());
			int count = 0;
			while (!(actionMap.get(1) && actionMap.get(2) && actionMap.get(3)
					&& actionMap.get(7) && actionMap.get(8) && actionMap.get(9)
					&& actionMap.get(11) && actionMap.get(12))) {
				Thread.sleep(500);
			...
			fireActions(workQueue, sys, "AA6_", EActionPriority.MINOR, 6);
			Assert.assertEquals(1, workQueue.countPendingActions());
			count = 0;
			while (!(actionMap.get(1) && actionMap.get(2) && actionMap.get(3)
					&& actionMap.get(4) && actionMap.get(5) && actionMap.get(11))) {
				Thread.sleep(500);
				if (++count > 20) { // wait 10s max
					Assert.fail("No response received from action threads");
					break;
