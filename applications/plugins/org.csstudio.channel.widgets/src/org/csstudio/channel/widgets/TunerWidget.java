...
public class TunerWidget extends AbstractChannelQueryResultWidget {
	...
	private void draw() {
		...
		channelTableViewer = new TableViewer(this, SWT.BORDER
				| SWT.FULL_SELECTION);
		table = channelTableViewer.getTable();
		table.setLinesVisible(true);
		table.setHeaderVisible(true);
		FormData fd_table = new FormData();
		fd_table.bottom = new FormAttachment(100, -70);
		fd_table.right = new FormAttachment(60);
		...
		setpointTableViewer = new TableViewer(this, SWT.BORDER
				| SWT.FULL_SELECTION);
		setpointTableViewer
		...
		fd_stepValueTable.bottom = new FormAttachment(table, 0, SWT.BOTTOM);
		fd_stepValueTable.right = new FormAttachment(100, -2);
		fd_stepValueTable.left = new FormAttachment(table, 2);
		fd_stepValueTable.top = new FormAttachment(0, 2);
		stepValueTable.setLayoutData(fd_stepValueTable);
		updateStepValueTable();

		Label lblStepCount = new Label(this, SWT.NONE);
		FormData fd_lblStepCount = new FormData();
		fd_lblStepCount.left = new FormAttachment(table, 2, SWT.LEFT);
		fd_lblStepCount.top = new FormAttachment(table, 5);
		...
		fd_stepCount.top = new FormAttachment(table, 5);
		...
		fd_lblStepSize.top = new FormAttachment(table, 5);
		...
		fd_stepSize.top = new FormAttachment(table, 5);
		...
		fd_btnGenerate.top = new FormAttachment(table, 5);
		...
		fd_btnApply.left = new FormAttachment(stepValueTable, 0, SWT.LEFT);
	...
	private void updateStepValueTable() {
		...
		if (tunerSetpointTableModel != null
				...
				&& tunerSetpointTableModel.getCalculatedSetpoints().size() > 0) {
			int columnCount = 1;
	...
	private List<String> getResultChannels(Result result) {
		...
		if (ex == null) {
			channelNames.addAll(ChannelUtil.getChannelNames(channels));
		}
