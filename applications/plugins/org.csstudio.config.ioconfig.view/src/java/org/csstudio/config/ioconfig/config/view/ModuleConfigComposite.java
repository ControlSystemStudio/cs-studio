 ...
 * @since 26.07.2007
 */
public class ModuleConfigComposite extends AbstractNodeConfig {
    ...
    private final class ISelectionChangedListenerForModuleTypeList implements
        ...
        private void makeNewChannel(final ModuleChannelPrototypeDBO channelPrototype, final int sortIndex) {
        ...
        private void makeStructChannel(final ModuleChannelPrototypeDBO channelPrototype, final int sortIndex) {
            ...
            ChannelStructureDBO channelStructure = ChannelStructureDBO.makeChannelStructure(_module,
        ...
        private void makeNewPureChannel(final ModuleChannelPrototypeDBO channelPrototype, final int sortIndex) {
            Date now = new Date();
            String createdBy = "Unknown";
            User user = SecurityFacade.getInstance().getCurrentUser();
            if (user != null) {
            ...
            ChannelStructureDBO cs = ChannelStructureDBO.makeSimpleChannel(_module, channelPrototype
...
//        new Label(topGroup, SWT.NONE).setText("Module Type: ");
    ...
    /**
     * Store all Data in {@link ModuleDBO} DB object.
     */
    ...
    public final AbstractNodeDBO getNode() {
        if (_module == null) {
            ...
            if (selection.getFirstElement() instanceof SlaveDBO) {
                SlaveDBO slave = (SlaveDBO) selection.getFirstElement();
                _module = new ModuleDBO(slave);
            } else  if(selection.getFirstElement() instanceof ModuleDBO) {
                ModuleDBO module = (ModuleDBO) selection.getFirstElement();
                _module = new ModuleDBO(module.getSlave());
            }
        }
        return _module;
    }
     ...
     */
    private GSDModuleDBO openChannelConfigDialog(final GsdModuleModel model, GSDModuleDBO gsdModule) {
        if (gsdModule == null) {
            gsdModule = new GSDModuleDBO(model.getName());
        }
        if(_module!=null) {
