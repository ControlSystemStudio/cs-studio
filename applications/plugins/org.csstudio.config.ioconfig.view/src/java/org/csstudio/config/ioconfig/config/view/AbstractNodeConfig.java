 ...
 */
/*
 * $Id: NodeConfig.java,v 1.11 2010/08/20 13:32:58 hrickens Exp $
 */
 ...
 * @since 23.07.2007
 */
public abstract class AbstractNodeConfig extends Composite implements INodeConfig {
    ...
    private final class IconChooserDialog extends Dialog {
        ...
        private IconChooserDialog(final Shell parentShell, final AbstractNodeDBO node) {
            super(parentShell);
    ...
    private final class CancelSelectionListener implements SelectionListener {
        ...
        public void widgetSelected(final SelectionEvent e) {
            if (getNode().isPersistent()) {
                ...
                final boolean openQuestion = MessageDialog.openQuestion(getShell(), "Cancel",
     ...
     *
     */
    public AbstractNodeConfig(final Composite parent, final ProfiBusTreeView profiBusTreeView,
            final String headline, final AbstractNodeDBO node, final boolean nevv) {
     ...
     */
    public abstract boolean fill(GSDFileDBO gsdFile);

    /**
     *
     * @return the GSD File of this Node.
     */
    public abstract GSDFileDBO getGSDFile();

    /**
     * Store the Header Data in {@link AbstractNodeDBO} DB object.
     */
    public void store() {
        // Header
        final Date now = new Date();
        final SimpleDateFormat df = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss.S");
        final Text text = (Text) this.getData("version");
        int i = -1;
        try {
     ...
     */
    private void setBackgroundComposite(final String headline, final AbstractNodeDBO node) {
        final int columnNum = 5;
        this.setLayout(new GridLayout(columnNum, true));
        final Label headlineLabel = new Label(this, SWT.NONE);
        if (_font == null) {
            _font = headlineLabel.getFont();
        }
        if (_fontBold == null) {
            final FontData fontData = _font.getFontData()[0];
        ...
        iconButton.addMouseListener(new MouseAdapter() {
            ...
            public void mouseUp(final MouseEvent e) {
                ...
                final IconChooserDialog chooseIconDialog = new IconChooserDialog(AbstractNodeConfig.this
    ...
    private static Label getNewLabel(final Group header, final String string) {
    ...
    private static Text getNewText(final Group header, final String text) {
    ...
    private Group makeBackgroundGroup(final int columnNum) {
        final Group header = new Group(this, SWT.H_SCROLL);
    ...
    protected final Composite getNewTabItem(final String head, final int rows) {
        final TabItem item = new TabItem(getTabFolder(), SWT.NONE);
        item.setText(head);
    ...
    protected boolean newNode(final String nameOffer) {

        final String nodeType = getNode().getClass().getSimpleName();

        final InputDialog id = new InputDialog(getShell(), "Create new " + nodeType,
            ...
            if (getNode() instanceof FacilityDBO || obj == null) {
                ...
                if (getNode().getParent() == null) {
                    ...
                    getNode().moveSortIndex(
                            nodeParent.getfirstFreeStationAddress(AbstractNodeDBO.MAX_STATION_ADDRESS));
     ...
     */
    protected final void setNameWidget(final Text nameText) {
        _nameText = nameText;
     ...
     *            set the Node Name description field.
     */
    protected final void setDescWidget(final Text descText) {
        _descText = descText;
    ...
    protected void makeDescGroup(final Composite comp, final int hSize) {
     ...
     */
    protected final void setProfiBusTreeView(final ProfiBusTreeView profiBusTreeView) {
