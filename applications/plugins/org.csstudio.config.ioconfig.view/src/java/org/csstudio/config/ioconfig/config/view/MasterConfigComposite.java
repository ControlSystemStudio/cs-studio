 ...
 * @since 06.06.2007
 */
public class MasterConfigComposite extends AbstractNodeConfig {
...
//        Composite comp = ConfigHelper.getNewTabItem(head, getTabFolder(), 6, 650, 440);
        ((GridLayout)comp.getLayout()).makeColumnsEqualWidth=false;
        makeNameGroup(comp,4);
        makeRedundencyMasterGroup(comp,2);        makeParametersGroup(comp);                makeMemoryAddressingGroup(comp);
    ...
    /**
     * Store all Data in {@link MasterDBO} DB object.
     */
    ...
    public final AbstractNodeDBO getNode() {
        if (_master == null) {
            ...
            if (selection.getFirstElement() instanceof ProfibusSubnetDBO) {
                ProfibusSubnetDBO profibusSubnet = (ProfibusSubnetDBO) selection.getFirstElement();
                _master = new MasterDBO(profibusSubnet);
            }
        }
        return _master;
    }
