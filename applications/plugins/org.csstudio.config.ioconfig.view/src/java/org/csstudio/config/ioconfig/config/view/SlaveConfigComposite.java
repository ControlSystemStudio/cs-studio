 ...
 * @since 05.06.2007
 */
public class SlaveConfigComposite extends AbstractNodeConfig {
    ...
    /**
     * Open a slave configuration view for a exist {@link SlaveDBO}.
     *
     * @param parent
     *            Parent Composite.
     * @param slave
    ...
    public SlaveConfigComposite(final Composite parent, final ProfiBusTreeView profiBusTreeView,
            final SlaveDBO slave) {
        this(parent,profiBusTreeView,slave,"");
    }
    public SlaveConfigComposite(final Composite parent, final ProfiBusTreeView profiBusTreeView,
            final SlaveDBO slave, final String nameOffer) {
     ...
     *            the Profibus Slave to Configer.
     */
    private void makeSlaveKonfiguration(final Composite parent, final SlaveDBO slave, final String nameOffer) {
        boolean nevv = false;
        _slave = slave;
        if (_slave == null) {
...
//        makeCurrentUserParamData(topGroup);
    ...
    /**
     * Store all Data in {@link SlaveDBO} DB object.
     */
    ...
    public AbstractNodeDBO getNode() {
        if (_slave == null) {
            ...
            if (selection.getFirstElement() instanceof MasterDBO) {
                MasterDBO master = (MasterDBO) selection.getFirstElement();
                _slave = new SlaveDBO(master);
                _slave.moveSortIndex((short) 0);
            } else if (selection.getFirstElement() instanceof SlaveDBO) {
                SlaveDBO slave = (SlaveDBO) selection.getFirstElement();
                _slave = new SlaveDBO(slave.getProfibusDPMaster());
