...
public class CallNewChildrenNodeEditor extends AbstractCallNodeEditor {
     ...
     */
    @Override
    protected void abcde(final Node parentNode,@Nonnull final IWorkbenchPage page) throws PartInitException {
        Node node = null;
        String id = null;
        String nodeType = "";
        if (parentNode instanceof Facility) {
            id = IocEditor.ID;
            node = new Ioc((Facility)parentNode);
            nodeType = "Ioc";
        }else if (parentNode instanceof Ioc) {
            id = SubnetEditor.ID;
            node = new ProfibusSubnet((Ioc) parentNode);
            nodeType = "Subnet";
        } else if (parentNode instanceof ProfibusSubnet) {
            id = MasterEditor.ID;
            node = new Master((ProfibusSubnet) parentNode);
            nodeType = "Master";
        } else if (parentNode instanceof Master) {
            id = SlaveEditor.ID;
            node = new Slave((Master) parentNode);
            nodeType = "Slave";
        } else if (parentNode instanceof Slave) {
            id = ModuleEditor.ID;
            node = new Module((Slave) parentNode);
            nodeType = "Module";
        }
