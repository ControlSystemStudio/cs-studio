 ...
 * @since 26.10.2011
 */
public abstract class AbstractBargraphBehaviorTest<B extends AbstractBehavior<BargraphModel>> extends AbstractBehaviorTest<BargraphModel, B> {


    @Override
    protected BargraphModel createModelMock() {
        final BargraphModel mock = mock(BargraphModel.class);
        when(mock.getColor(AbstractWidgetModel.PROP_COLOR_FOREGROUND)).thenReturn("UserDefinedColor");
        when(mock.getColor(BargraphModel.PROP_FILLBACKGROUND_COLOR)).thenReturn("UserDefinedColor");
        return mock;
    }
...
//        getInOrder().verify(getModelMock()).setPropertyValue(BargraphModel.PROP_TRANSPARENT,false);
    ...
    protected void verifyConnectionStateConnecting() {
        getInOrder().verify(getModelMock()).setPropertyValue(AbstractWidgetModel.PROP_BORDER_STYLE,BorderStyleEnum.DASH_DOT);
        getInOrder().verify(getModelMock()).setPropertyValue(AbstractWidgetModel.PROP_BORDER_WIDTH,1);
        getInOrder().verify(getModelMock()).setPropertyValue(AbstractWidgetModel.PROP_BORDER_COLOR,"${Initial}");
        getInOrder().verify(getModelMock()).setPropertyValue(BargraphModel.PROP_FILLBACKGROUND_COLOR,"${Initial}");
    ...
    protected void verifyConnectionStateFailed() {
        getInOrder().verify(getModelMock()).setPropertyValue(AbstractWidgetModel.PROP_BORDER_STYLE,BorderStyleEnum.DASH_DOT);
        getInOrder().verify(getModelMock()).setPropertyValue(AbstractWidgetModel.PROP_BORDER_WIDTH,1);
        getInOrder().verify(getModelMock()).setPropertyValue(AbstractWidgetModel.PROP_BORDER_COLOR,"${VerbAbbr}");
        getInOrder().verify(getModelMock()).setPropertyValue(BargraphModel.PROP_FILLBACKGROUND_COLOR,"${VerbAbbr}");
...
//        getInOrder().verify(getModelMock()).setPropertyValue(BargraphModel.PROP_TRANSPARENT,true);
    ...
    //        verify(_polygonModelMock, times(2)).setColor(AbstractWidgetModel.PROP_COLOR_FOREGROUND, "${Zu}");
