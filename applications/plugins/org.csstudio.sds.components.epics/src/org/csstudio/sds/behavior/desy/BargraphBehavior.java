...
public class BargraphBehavior extends AbstractDesyBehavior<BargraphModel> {
	...
	protected void doInitialize(BargraphModel widget) {
		// .. border
		widget.setPropertyValue(AbstractWidgetModel.PROP_BORDER_STYLE, determineBorderStyle(ConnectionState.INITIAL));
		widget.setPropertyValue(AbstractWidgetModel.PROP_BORDER_WIDTH, determineBorderWidth(ConnectionState.INITIAL));
		widget.setPropertyValue(AbstractWidgetModel.PROP_BORDER_COLOR, determineBorderColor(ConnectionState.INITIAL));
	}
	...
	protected void doProcessValueChange(BargraphModel widget, AnyData anyData) {
		...
		widget.setPropertyValue(BargraphModel.PROP_DEFAULT_FILL_COLOR, determineColorBySeverity(anyData.getSeverity()));
	}
	...
	protected void doProcessConnectionStateChange(BargraphModel widget, org.epics.css.dal.context.ConnectionState connectionState) {
		// .. border
		widget.setPropertyValue(AbstractWidgetModel.PROP_BORDER_STYLE, determineBorderStyle(connectionState));
		widget.setPropertyValue(AbstractWidgetModel.PROP_BORDER_WIDTH, determineBorderWidth(connectionState));
		widget.setPropertyValue(AbstractWidgetModel.PROP_BORDER_COLOR, determineBorderColor(connectionState));
		
		// .. background colors
		widget.setPropertyValue(BargraphModel.PROP_FILLBACKGROUND_COLOR, determineBackgroundColor(connectionState));
		widget.setPropertyValue(BargraphModel.PROP_COLOR_BACKGROUND, determineBackgroundColor(connectionState));
