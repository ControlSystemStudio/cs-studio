 ...
 * @since 20.04.2010
 */
public class TextinputConnectionExpDblSwitchBehavior extends AbstractDesyConnectionBehavior<TextInputModel> {
     ...
     * Constructor.
     */
    public TextinputConnectionExpDblSwitchBehavior() {
    ...
    protected void doInitialize(final TextInputModel widget) {
        ...
        widget.setJavaType(Double.class);
    ...
    protected void doProcessValueChange(final TextInputModel model, final AnyData anyData) {
        ...
        if (anyData.doubleValue() < EXP_DOUBLE_THRESHOLD) {
            model.setPropertyValue(TextInputModel.PROP_TEXT_TYPE, 4);
        } else {
            model.setPropertyValue(TextInputModel.PROP_TEXT_TYPE, 1);
            model.setPropertyValue(TextInputModel.PROP_PRECISION, 5);
        }
