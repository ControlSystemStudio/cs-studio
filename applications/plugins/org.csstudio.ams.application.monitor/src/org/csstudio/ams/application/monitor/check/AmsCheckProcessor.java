...
import org.csstudio.ams.application.monitor.message.AmsAnswerMessage;
 ...
 * @since 10.04.2012
 */
public class AmsCheckProcessor extends AbstractCheckProcessor implements MessageListener {
    
    private static final Logger LOG = LoggerFactory.getLogger(AmsCheckProcessor.class);

    private Vector<AmsAnswerMessage> messageQueue;
    
    private long waitTime;

    public AmsCheckProcessor(String subscriberName, String ws) {
        this(subscriberName, ws, 0L);
    }
    
    public AmsCheckProcessor(String subscriberName, String ws, long interval) {
        ...
        if (!readMonitorStatus()) {
            monitorStatus = new MonitorStatus(AmsMonitorPreference.MAX_ALLOWED_AMS_ERROR.getValue(),
                                              0);
        } else {
    ...
    public void run() {
        ...
        InitiatorMessage currentMessage = messageCreator.getInitiatorMessage(MessageType.AMS);
        ...
        try {
            ...
            MapMessage mapMessage = messageConverter
                                     .convertToMapMessage(currentMessage, publisher.getSession());
        ...
        // Now check the received messages
        Enumeration<AmsAnswerMessage> elements = messageQueue.elements();
        while (elements.hasMoreElements()) {
            AmsAnswerMessage o = elements.nextElement();
            if (o.isAnswerForMessage(currentMessage)) {
                LOG.info("Success: It is the answer for the current message.");
                monitorStatus.forgetAll();
                currentStatus.setCheckStatus(CheckStatus.OK);
                currentStatus.setErrorReason(ErrorReason.AMS);
    ...
    public synchronized void onMessage(Message message) {
        ...
        AmsAnswerMessage aam = messageConverter.convertToAmsAnswerMessage(mm);
        // LOG.debug("{}", aam.toString());
        if (aam.isValid()) {
            messageQueue.add(aam);
