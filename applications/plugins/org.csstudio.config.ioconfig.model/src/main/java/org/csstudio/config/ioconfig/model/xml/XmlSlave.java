...
public class XmlSlave {
     ...
     *  
     * @param slave The Profibus slave. 
     */
     ...
     * @param slave The Profibus Slave.
     * @param prmDataLen The parameter data length. 
     * @param cfgDataLen The Config data length.
     * @param slaveAatLen The Slave aat length.
     * @param slaveUserDataLen The user data length.
     * @return The XML Slave Para Set Element.
     */
    private Element setSlaveParaSet(final Slave slave, final int prmDataLen, final int cfgDataLen, final int slaveAatLen, final int slaveUserDataLen) {
    ...
    public final Element setSlavePrmData(final Slave slave) {
        ...
        String prmData ="";
        if(slave.getGSDSlaveData()!=null && slave.getGSDSlaveData().getExtUserPrmDataConst()!=null){
            prmData = slave.getGSDSlaveData().getExtUserPrmDataConst().values().toString().replaceAll("[\\[\\]]", "");
            for (Module module : _modules) {
                String modiExtUserPrmDataConst = module.getGsdModuleModel().getModiExtUserPrmDataConst();
                if(modiExtUserPrmDataConst!=null&&modiExtUserPrmDataConst.length()>0){
        ...
        if(slaveData!=null){
             ...
             */
            slavePrmData.setAttribute("ident_number", "0x"+Integer.toHexString(slaveData.getIdentNumber()));
        }
