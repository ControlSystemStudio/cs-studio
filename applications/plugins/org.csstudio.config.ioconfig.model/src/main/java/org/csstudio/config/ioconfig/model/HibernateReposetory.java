...
public class HibernateReposetory implements IReposetory {
    ...
    private static final Logger LOG = CentralLogger.getInstance()
            .getLogger(HibernateReposetory.class);
    ...
    private final class EpicsAddressHibernateCallback implements HibernateCallback {
        ...
        public String execute(final Session session) {
            ...
            query.setString(0, Channel.class.getName()); // Zero-Based!
    ...
    @Override
    @CheckForNull
    public Channel loadChannel(@Nullable final String ioName) {
        HibernateCallback hibernateCallback = new HibernateCallback() {
    ...
    @Override
    @CheckForNull
    public List<PV2IONameMatcherModel> loadPV2IONameMatcher(@Nullable final Collection<String> pvName) {
        HibernateCallback hibernateCallback = new HibernateCallback() {
            public List<PV2IONameMatcherModel> execute(final Session session) throws HibernateException {
                if ( (pvName == null) || (pvName.size() == 0)) {
                    return null;
                }
                StringBuilder statement = new StringBuilder("select pv from ")
                        .append(PV2IONameMatcherModel.class.getName()).append(" pv where ");
