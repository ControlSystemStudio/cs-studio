...
public class LegendHandler<XTYPE extends Comparable<XTYPE>> {
	...
	boldDescriptor = FontDescriptor.createFrom(
	        (Display.getCurrent().getSystemFont())).setStyle(SWT.BOLD);
	// A font for the figureutilities to calculate the average width
	boldFont = boldDescriptor.createFont(null);
	...
	if (plot.getTraceCount() > 0) {
		...
		sum = sum
		        + FigureUtilities.getTextWidth(trace.getName(),
		                boldFont);
	    }
		...
		label.setForeground(new Color(label.getDisplay(), trace
		        .getColor()));
