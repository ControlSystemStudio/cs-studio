...
public final class FieldFunctionService implements IFieldFunctionService {
     ...
     * 
     * @return the resolved String in which all variables are replaced by their values
     */
    public String resolveVariables(final String source, final Map<String, String> aliases)
            throws AliasResolutionException {
        return doResolveVariablesRecursively(source, new ArrayList<String>(), false, aliases);
    }

    /**
     * This method is called recursively, to apply alias substitutions to the provided input text.
     * 
     * @param input
     *            the text input
     * @param markerList
     *            a marker list, which contains aliases that are already on the stack
     * @param isAlias
     *            flag, which indicates whether the input is a alias (this is necessary because
     *            aliases might contain other aliases which is handled via the same recursive call)
     * @param aliases
     *            the existing aliases as provided by the user
     * 
     * @return a canonical name in which all aliases are replaced by their real values
     * 
     * @throws AliasResolutionException
     */
     ...
     * 
     * @param aliasName
     *            the alias name, which should be found (without the bordering "$" signs
     * 
    ...
    public void registerFunction(String name, IFieldFunction function) {
        ...
        if (functions.containsKey(name)) {
            throw new IllegalArgumentException("Only 1 function can be registered for name \""
                    + name + "\"");
        }
     ...
     * @throws Exception
     */
    private String findAndApplyFunctions(String source, IRecord record, String fieldName)
            throws Exception {
     ...
     * @throws Exception
     */
    private String applyFunction(String functionName, String[] parameters, IRecord record,
