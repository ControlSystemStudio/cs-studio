 ...
 * <p>
 * NOTE: this class is extensible as per Bastian request so that DESY can hook a
 * different type factory. This is a temporary measure until the problem is
 * solved in better, more general way, so that data sources can work only with
 * data source specific types, while allowing conversions to normalized type
 * through operators. The contract of this class is, therefore, expected to
 * change.
 * <p>
 * Related changes are marked so that they are not accidentally removed in the
 * meantime, and can be intentionally removed when a better solution is
 * implemented.
 * 
 * @author carcassi
 */
public class JCAChannelHandler extends ChannelHandler<MonitorEvent> {
	...
	public JCAChannelHandler(String channelName, Context context,
			int monitorMask) {
		super(channelName);
	...
	@Override
	public synchronized void addMonitor(Collector<?> collector,
	 ...
	 * 
	 * @param cacheType
	 *            final type for the value update
	 * @param channel
	 *            channel access channel from which to get the data
	 * @return the TypeFactory to use
	 */
	protected TypeFactory matchFactoryFor(Class<?> cacheType, Channel channel) {
		return VTypeFactory.matchFor(cacheType, channel.getFieldType(),
		...
		try {
			...
			if (largeArray) {
				channel = context.createChannel(getChannelName(),
						connectionListener, Channel.PRIORITY_MIN);
				System.out.println("Connect as large");
			} else {
				channel = context.createChannel(getChannelName(),
						connectionListener, (short) (Channel.PRIORITY_MIN + 1));
			}
			needsMonitor = true;
	...
	protected void setup(Channel channel) throws CAException {
		...
		if (vTypeFactory.getEpicsMetaType() != null) {
			// Need to use callback for the listener instead of doing a
			// synchronous get
		...
		if (needsMonitor) {
			if (vTypeFactory.isArray()) {
				channel.addMonitor(vTypeFactory.getEpicsValueType(),
						channel.getElementCount(), monitorMask, monitorListener);
			} else {
				channel.addMonitor(vTypeFactory.getEpicsValueType(), 1,
						monitorMask, monitorListener);
			}
			needsMonitor = false;
		}
	...
	private final ConnectionListener connectionListener = new ConnectionListener() {
		...
		public void connectionChanged(ConnectionEvent ev) {
			try {
				...
				if (ev.isConnected()
						&& channel.getElementCount() >= LARGE_ARRAY
						&& !largeArray) {
	...
	private synchronized void dispatchValue() {
		...
		if (event != null
				&& (vTypeFactory.getEpicsMetaType() == null || metadata != null)) {
			processValue(event);
		}
	}
	...
	public void write(Object newValue, final ChannelWriteCallback callback) {
		try {
			...
			if (newValue instanceof String) {
				...
				throw new RuntimeException("Unsupported type for CA: "
						+ newValue.getClass());
			}
			context.flushIO();
		} catch (CAException ex) {
	...
	public boolean updateCache(MonitorEvent event, ValueCache<?> cache) {
		...
		Object newValue = cacheType.cast(vTypeFactory.createValue(rawvalue,
	...
	public boolean isConnected() {
		return channel != null
				&& channel.getConnectionState() == Channel.ConnectionState.CONNECTED;
	}
}
