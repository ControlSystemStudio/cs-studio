...
public class MessageListAutoStart implements IStartupServiceListener {
	...
	/**
	 * This method is executed by the startup listener from platform at CSS
	 * startup. The preferences are read for each table type and the message
	 * lists are started for each list with the preference 'auto start' == true.
	 */
	@Override
	public void run() {
	...
	private void startLogLists(String maximumNumberOfMessagesPref) {
		TopicSetColumnService topicSetColumnService = new TopicSetColumnService(
		...
		for (TopicSet topicSet : topicSets) {
			if (topicSet.isStartUp()) {
				_messageListService.initializeLogMessageList(topicSet, Integer.parseInt(maximumNumberOfMessagesPref));
			}
		}
	}
