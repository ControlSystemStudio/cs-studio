...
/**
 * This class handles the receiver jms connection (via the shared jsm connection in css platform)
 * New messages are added to the model (_messageList).
 * 
 * @author jhatje
 * 
 */
public class JmsMessageReceiver implements MessageListener {
        ...
        if (message == null) {
            ...
            try {
                if (message instanceof MapMessage) {
                    ...
                    JmsLogsPlugin.logError("received message is of unknown type: "
                            + message.getClass().getName());
                }
            } catch (Exception e) {
                JmsLogsPlugin.logException("Error parsing message: ", e); //$NON-NLS-1$
            }
        }
    }
    ...
    private Map<String, String> readMapMessageProperties(final MapMessage mm) throws JMSException {
    ...
    /**
     * Start jms message listener. If there is a previous session active (the user has edited the
     * topics) it will be closed and a new session is created.
     * 
     * @param _deafultTopicSet
     *            JMS topics to be monitored
     */
    public void initializeJMSConnection(final List<String> list, final MessageList messageList) {
        _messageList = messageList;
        String[] topicList = null;
        if ( (list == null) || (list.size() == 0)) {
            CentralLogger.getInstance()
                    .error(this, "Could not initialize JMS Listener. JMS topics == NULL!");
        } else {
        ...
        try {
            ...
            CentralLogger.getInstance()
                    .info(this, "Initialize JMS connection with topics: " + list);
