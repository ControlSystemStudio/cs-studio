...
public class ExpertSearchDialog extends Dialog implements CalendarWidgetListener,
    ...
    private class MsgType {
         ...
         * 
         * @param id
         *            set the Id of the MSg Type.
         * @param type
         *            set the name of the MSg Type.
         */
     ...
     * 
     * @param shell
     *            The Shell to Display this Dialog.
     * @param start
     *            The default start time or null for now.
     * @param end
     *            The default start time or null for now.
     * @param filter
     *            The default filtersor null for none.
     */
    public ExpertSearchDialog(final Shell shell, final ITimestamp start, final ITimestamp end,
            final ArrayList<FilterItem> filterConditions) {
        super(shell);
        this._shell = shell;
        this._start = start;
        this._end = end;
    ...
    protected final Control createDialogArea(final Composite parent) {
        ...
        Composite[] c2 = makeTimeSelect(box, Messages.ExpertSearchDialog_startTime,
                ...
                org.csstudio.apputil.ui.time.Messages.StartEnd_RelStart_TT, _start.toCalendar());
        ...
        c2 = makeTimeSelect(box, Messages.ExpertSearchDialog_endTime,
                ...
                org.csstudio.apputil.ui.time.Messages.StartEnd_RelEnd_TT, _end.toCalendar());
        ...
        // }
        // }
        if (_filterConditions != null && _filterConditions.size() > 0) {
            for (FilterItem filterItem : _filterConditions) {
                addNewFilter(down, filterItem.getProperty(), filterItem.getValue(), filterItem
                        .getRelation());
            }
        } else {
            addNewFilter(down, null, null, null);
     ...
     * @param absTime
     *            the Text for the tab from abs
     * @param absTimeTT
     *            the Tooltip text for the tab from abs
     * @param relTime
     *            the Text for the tab from rel
     * @param relTimeTT
     *            the Tooltip text for the tab from rel
     * @return CalendarWidget absWidget and RelativeTimeWidget relWidget
     */
    private Composite[] makeTimeSelect(final Composite box, final String groupName,
            final String absTime, final String absTimeTT, final String relTime,
            final String relTimeTT, Calendar defaultTime) {
        TabFolder leftTab = new TabFolder(timeBox(box, groupName), SWT.BORDER);
        TabItem tab = new TabItem(leftTab, 0);
        tab.setText(absTime);
        tab.setToolTipText(absTimeTT);
        CalendarWidget absWidget = new CalendarWidget(leftTab, 0, defaultTime);
     ...
     * 
     * @param parent
     *            The parent Group to add the row.
     * @param type
     *            The selectet Type.
     * @param value
     *            The default vaule to the Typ.
     * @param logic
     *            The logic is set AND, OR or "" for not set
     */
    private void addNewFilter(final Group parent, final String type, final String value,
            final String logic) {
        ...
        if (logic == null || logic.trim().length() == 0||logic.equals("end")) {
     ...
     * 
     * @param text
     *            The value text for the filter.
     * @param parent
     *            The parent Composit.
     * @param comButton
     *            The Composit to contain the Buttons.
     * @param newRow
     *            if true add a new row.
     */
    private void addRow(final String text, final Composite parent, final Composite comButton,
    ...
    protected final void okPressed() {
        ...
        for (int i = 0; i < rowsComposite.length; i++) {
            ...
            if (rowsComposite[i] instanceof Composite) {
                ...
                if (typeAndValueComposite.length == 2
                        && typeAndValueComposite[0] instanceof Composite
                        && typeAndValueComposite[1] instanceof Composite) {
                    Control[] typeComboAndValueText = ((Composite) typeAndValueComposite[0])
                            .getChildren();

                    FilterItem filterSetting = new FilterItem();

                    // First part a Composite with a Combo for the Typ and a Text for the value.
                    if (typeComboAndValueText[0] instanceof Combo
                            && typeComboAndValueText[1] instanceof Text) {
                        filterString += " (lower(mpt.NAME) like lower('" //$NON-NLS-1$
                                + ((Combo) typeComboAndValueText[0])
                                        .getItem(((Combo) typeComboAndValueText[0])
                                                .getSelectionIndex()) + "')" //$NON-NLS-1$
                                + " AND lower(mc.VALUE) like lower('" //$NON-NLS-1$
                                + ((Text) typeComboAndValueText[1]).getText() + "'))"; //$NON-NLS-1$
                        filterSetting.set_property(((Combo) typeComboAndValueText[0])
                                .getItem(((Combo) typeComboAndValueText[0]).getSelectionIndex()));
