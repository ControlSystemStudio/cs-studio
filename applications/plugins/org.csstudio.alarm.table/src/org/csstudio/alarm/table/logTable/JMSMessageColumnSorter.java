...
public class JMSMessageColumnSorter extends ViewerSorter {
	...
	public int compare(Viewer viewer, Object o1, Object o2) {
		...
		switch(type){
			case 0: if(einsInt==zweiInt){
						return bw*collator.compare(jmsm1.getProperty("SEVERITY"),
								jmsm2.getProperty("SEVERITY"));
					}else return bw*(einsInt-zweiInt);
			case 1: if(einsF==zweiF){
						return bw*collator.compare(jmsm1.getProperty("SEVERITY"),
								jmsm2.getProperty("SEVERITY"));
					}else return (int)(bw*(einsF-zweiF));
			default:int comp = collator.compare(s1, s2);
					if (comp == 0) {
						return bw*collator.compare(jmsm1.getProperty("SEVERITY"),
								jmsm2.getProperty("SEVERITY"));
					} else if (s1.length()==0){
