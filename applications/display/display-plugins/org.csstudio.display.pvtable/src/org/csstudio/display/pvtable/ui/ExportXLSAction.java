...
public class ExportXLSAction extends PVTableAction {
	...
	public void run() {
		...
		if(model.getItemCount()==0) {
			MessageDialog.openInformation(Display.getCurrent().getActiveShell(), Messages.InformationPopup, Messages.EmptyTable);
			return;
		}
		...
		String pvTableTitle = PlatformUI.getWorkbench().getActiveWorkbenchWindow().getActivePage().getActiveEditor().getTitle();
			...
			for (int a = 2, b = 0 ; b < measures.size() ; a++, b++) {
				...
				for(int d = 1 ; d < measures.get(b).getItems().size() ; d++) {
					...
					if(measureMap.get(measures.get(b).getItems().get(d).getName()) == null) {
						// On ajoute les données de la PV non présente dans la map dans les 2 premières lignes.
