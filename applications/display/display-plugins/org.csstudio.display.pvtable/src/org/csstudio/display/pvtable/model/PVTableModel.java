...
public class PVTableModel implements PVTableItemListener {
    ...
    /** Add table item
     *  Check if the item is a conf or not
    ...
    /**
     * Add a new item above the selected row.
     * And check if this new item is added to a configuration.
     * @param item
     * @param newItem
     * @return newItem
     */
     ...
     *  If multiple items changed, refresh the whole table.</p>
     */
    ...
    /**
     * Check if the item is a confHeader, if a configuration already exist and create it if not.
     * @param item
     */
        	...
        	if(config == null) {
        		...
        		for(PVTableItem i : items) {
            		if(i.isConfHeader()) {
            			...
            			MessageDialog.openInformation(Display.getCurrent().getActiveShell(), Messages.InformationPopup, Messages.InformationPopup_ConfAlreadyExist);
    ...
    */
    ...
    /** Save snapshot value of each item from the conf
     *  @param items Items to save
     */
