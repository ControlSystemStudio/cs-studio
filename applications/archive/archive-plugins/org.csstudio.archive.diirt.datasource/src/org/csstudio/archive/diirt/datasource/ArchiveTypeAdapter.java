 ...
 *
 */
public class ArchiveTypeAdapter implements DataSourceTypeAdapter<Boolean, ArchiveMessagePayload> {
    ...
    @Override
    public boolean updateCache(ValueCache cache, Boolean connection, ArchiveMessagePayload message) {
        List<VType> values = message.values;
        if (values.isEmpty()) {
            VTable table = ValueFactory.newVTable(Arrays.asList(VType.class), Arrays.asList(VALUES),
                Arrays.asList(values));
            cache.writeValue(table);
        } else {
            // users of this plugin should not be required to import the archive.vtype plugin. Therefore, transform
            // the archive vtypes to standard vtypes
