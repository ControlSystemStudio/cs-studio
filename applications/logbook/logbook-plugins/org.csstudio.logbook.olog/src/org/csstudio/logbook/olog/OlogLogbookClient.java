...
public class OlogLogbookClient implements LogbookClient {
    ...
    private final List<String> levels = Arrays.asList("Info", "Problem",
            "Request", "Suggestion", "Urgent");
    ...
    public Collection<Logbook> listLogbooks() throws Exception {
        return Collections.unmodifiableCollection(
                Collections2.transform(reader.listLogbooks(),
                        new Function<edu.msu.nscl.olog.api.Logbook, Logbook>() {

                            @Override
                            public Logbook apply(
                                    edu.msu.nscl.olog.api.Logbook input) {
    ...
    public Collection<Tag> listTags() throws Exception {
        return Collections.unmodifiableCollection(
                Collections2.transform(reader.listTags(),
                    ...
                    public Attachment apply(
                        ...
                        try {
                            return new OlogAttachment(input,
                                    getAttachment(logId, input.getFileName()));
                        } catch (IOException e) {
                        }
                        return null;
                    }
    ...
    public Collection<LogEntry> findLogEntries(String search) throws Exception {
        ...
        if (searchParameters.containsKey(SEARCH_KEYWORD_START)) {
            ...
            if (startEndTimeParser != null && startEndTimeParser.getStart() != null
                ...
                searchParameters.put("start", String.valueOf(startEndTimeParser
                        .getStart().toInstant().getEpochSecond()));
                searchParameters.put("end", String.valueOf(startEndTimeParser
    ...
    public LogEntry updateLogEntry(LogEntry logEntry) throws Exception {
        OlogEntry ologEntry = new OlogEntry(
                writer.update(LogBuilder(logEntry)));
        // creates the log entry and then adds all the attachments
        // TODO (shroffk) multiple network calls, one for each attachment, need
        // to improve
        Collection<String> existingFiles = new ArrayList<String>();
        for (edu.msu.nscl.olog.api.Attachment attachment : reader
                .getLog((Long) ologEntry.getId()).getAttachments()) {
            existingFiles.add(attachment.getFileName());
        }
        for (Attachment attachment : logEntry.getAttachment()) {
            // Check the attachment doe snot already exist.
            if (!existingFiles.contains(attachment.getFileName())
        ...
        try {
            ...
            if (file != null) {
                ...
                return new OlogAttachment(response,
                        getAttachment(logId, response.getFileName()));
            }
            return null;
        } catch (IOException e) {
    ...
    private LogBuilder LogBuilder(LogEntry logEntry) {
        LogBuilder logBuilder = log().description(logEntry.getText())
                .level(logEntry.getLevel()).id((Long) logEntry.getId());
        for (Tag tag : logEntry.getTags())
            logBuilder.appendTag(tag(tag.getName(), tag.getState()));
        for (Logbook logbook : logEntry.getLogbooks())
            logBuilder.appendToLogbook(
                    logbook(logbook.getName()).owner(logbook.getOwner()));
    ...
    private class OlogEntry implements LogEntry {
        ...
        public OlogEntry(edu.msu.nscl.olog.api.Log log) {
            ...
            this.attachments = Collections2.transform(log.getAttachments(),
                    new Function<edu.msu.nscl.olog.api.Attachment, Attachment>() {
