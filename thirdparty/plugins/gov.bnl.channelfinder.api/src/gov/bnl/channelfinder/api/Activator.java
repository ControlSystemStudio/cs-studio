        ...
        try {
            IConfigurationElement[] config = Platform.getExtensionRegistry()
                    .getConfigurationElementsFor(
                            "gov.bnl.channelfinder.api.client");
            ...
            if (config.length == 1) {
                ChannelFinderClient client = (ChannelFinderClient) config[0]
                        .createExecutableExtension("channelfinderclient");
                return client;
            }

            throw new IllegalStateException(
                    "More than one ChannelFinderClient was configured through extensions.");
        } catch (Exception e) {
